...
description: "The Razorpay Remote MCP Server is a hosted solution that provides zero infrastructure overhead with automatic updates and high availability. This is the **recommended deployment option** for most users."
---
...
- **Authentication**: Generate [Razorpay API keys](/payments/dashboard/account-settings/api-keys#generate-api-keys) from the Dashboard. Alternatively, you can use [OAuth](/mcp-server/oauth).
...
<Steps>
<Step title="Log in to your [Razorpay Dashboard](https://dashboard.razorpay.com/).">
Log in to your [Razorpay Dashboard](https://dashboard.razorpay.com/).
</Step>
<Step title="Navigate to **Account & Settings** â†’ **API Keys**.">
Navigate to **Account & Settings** â†’ **API Keys**.
</Step>
<Step title="Generate your API keys (Key ID and Key Secret).">
Generate your API keys (Key ID and Key Secret).
</Step>
<Step title="Copy both the Key ID and Key Secret.">
...
Effective **August 13, 2025**, the endpoint `https://mcp.razorpay.com/sse` is deprecated and replaced by `https://mcp.razorpay.com/mcp`, which supports streamable HTTP responses for improved performance. Update your integrations immediately to avoid service disruptions. Refer to the [changelog](/api/changelog).
...
### Cursor Configuration
      ...
      1. Open **Cursor**.
      2. Navigate to **Settings** â†’ **Cursor Settings**.
...
<Steps>
<Step title="**Explore Use Cases**: Review [practical examples](/mcp-server/use-...">
**Explore Use Cases**: Review [practical examples](/mcp-server/use-cases) of AI-powered payment operations.
</Step>
<Step title="**Learn Available Tools**: Check the [Tools Reference](/mcp-server/...">
**Learn Available Tools**: Check the [Tools Reference](/mcp-server/tools-reference) for complete capabilities.
</Step>
<Step title="**Advanced Configuration**: See [Configuration Guide](/mcp-server/c...">
**Advanced Configuration**: See [Configuration Guide](/mcp-server/configuration) for additional options.
</Step>
<Step title="**Start Building**: Begin using natural language commands to intera...">
