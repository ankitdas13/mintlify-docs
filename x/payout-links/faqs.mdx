...
description: "No, currently it is not supported in [Test mode](/x/dashboard/test-mode). You can only make requests or designate links in Live mode."
---
...
### 1. Is the Payout Link supported in test mode?
         ...
         No, currently it is not supported in [Test mode](/x/dashboard/test-mode). You can only make requests or designate links in Live mode.
...
### 2. What are the supported modes for creating Payout Links?
         ...
         You can create Payout Links via:
            - [Payout Link APIs](/api/x/payout-links).
            - [RazorpayX Dashboard](https://x.razorpay.com/).
            - [Bulk upload feature](/x/payout-links/bulk).
...
### 4. Is the maker-checker applicable for Payout Links?
         ...
         Yes, if the [maker-checker](/x/manage-teams/approval-workflow) is enabled on the account, then it will be applicable. 
...
### 7. If a new link is issued to an old customer, will the customer need to enter the fund account details again?
         ...
         You can manually [resend the Payout Link](/x/payout-links/create#create-a-payout-link) from the Payout Link summary pop-up page. 
        

    
### 8. Can a Payout link be cancelled?

         If the Fund Account details are incorrect or the Payout Link is no longer relevant, you can cancel the Payout Link via the [Dashboard](/x/payout-links/create#other-actions) or by using the [Cancel a Payout Link API](/api/x/payout-links/cancel).

         Only Payout Links in the `issued` state can be cancelled. Know more about [Payout Link Statuses](/x/payout-links/life-cycle). If [Approval Workflow](/x/manage-teams/approval-workflow) is enabled, [cancel the Payout](/x/payouts#cancel-payouts) first and then the Payout Link. 
...
### 11. Does the Payout Link support an expiry time?
         ...
         Yes, you can [set expiry](/x/payout-links/set-expiry#set-expiry) date and time on the Payout Link.
...
### 14. Is IP Allowlisting mandatory for creating Payout Links?
        ...
        [Allowlisting your IP](/x/dashboard/allowlist-ip) addresses is mandatory for creating Payout Links through APIs. However, it is not mandatory for creating Payout Links through the Dashboard.
...
### 2. How can I set expiry for Bulk Payout Links?
         ...
         To set expiry for Bulk Payout Links, you must [enable expiry](/x/payout-links/set-expiry#enable-expiry) on the RazorpayX Dashboard. You can then [set expiry](/x/payout-links/bulk#how-it-works) for bulk Payout Links in the sample template.
