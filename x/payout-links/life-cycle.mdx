---
title: "Payout Links Life Cycle"
description: "A Payout Link can have the following statuses during its life cycle:"
---
...
As soon as a Payout Link is created, it immediately moves to the `pending` state as shown in the image below. The Payout Link remains in this state until it is sent for approval. This status is applicable only for cases where [Approval Workflow](/x/manage-teams/approval-workflow) is enabled.
...
Know how you can [set Payout Links to expire](/x/payout-links/set-expiry) after a certain duration. 

### Rejected

A Payout Link moves to the `rejected` state if it is rejected by the Approver. This status is applicable only for cases where [Approval Workflow](/x/manage-teams/approval-workflow) is enabled.
...
Refer to [Payout Life Cycle and States](/x/payouts#payout-life-cycle) for more information.

### Related Information

- [About Payout Links](/x/payout-links)
- [Bulk Payout Links](/x/payout-links/bulk)
