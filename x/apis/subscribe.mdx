...
description: "Webhooks (Web Callback, HTTP Push API or Reverse API) are one of the ways in which one web application can send information to another application in real-time when a specific event happens. Know more about [Webhooks](/webhooks)."
---

## Set Up and Edit Payout Webhooks
You must first [set up and edit Payout Webhooks](/webhooks/setup-edit-payouts) as per your requirement.

## Validate and Test Webhooks
Then you must [validate and test webhooks](/webhooks/validate-test) as a best practice. 
...
When your webhook `secret` is set, Razorpay uses it to create a hash signature with each payload. This hash signature is passed with each request under the `X-Razorpay-Signature` header that you need to validate at your end. Support for validating the signature is provided in all our  [language SDKs](/payments/server-integration).
...
See [Payout Sample Payloads](/webhooks#payout-payloads).

For transaction webhook events, check the [Transaction Sample Payloads](/webhooks#transaction-payloads).

### Related Information

- [Set Up Webhooks for Payouts](/webhooks/setup-edit-payouts)
