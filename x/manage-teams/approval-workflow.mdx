...
description: "Approval Workflow, or the Maker-Checker, is a feature that can be used by organisations to have certain transactions go through an approval process."
---
...
Approval Workflow can be applied to all payouts created on RazorpayX, whether they are created individually from the [RazorpayX Dashboard](https://x.razorpay.com/), using [API](/api/x/payouts) or using the [Bulk Payout](/x/bulk-payouts#payouts) feature.
...
1. Confirm saving workflow and [2FA](/x/manage-teams/2fa) to create workflow successfully.
...
<Step title="Navigate to **My Account & Settings** → **Workflow** → **Payouts**.">
Navigate to **My Account & Settings** → **Workflow** → **Payouts**.
</Step>
<Step title="Click on the delete icon and click **Yes, Proceed**.">
Click on the delete icon and click **Yes, Proceed**.

</Step>
<Step title="Enter the OTP sent to your registered email id and mobile number an...">
Enter the OTP sent to your registered email id and mobile number and click **Confirm**.
</Step>
</Steps>
...
In addition to the [webhook events available for payouts](/webhooks), the Approval Workflow has the following events.
...
- [Invoices - Approval Workflow](/x/vendor-payments/approvals-on-invoices)
