---
title: "Payouts Best Practices"
description: "The following table lists the best practices for integrating RazorpayX payouts:"
---
...
1. Create a [Contact](/api/x/contacts/create). You receive a Contact id in the response.
1. Create a Fund account using the Contact id. The Fund account can be a [bank account](/api/x/fund-accounts/create/bank-account) or [VPA](/api/x/fund-accounts/create/vpa).
1. Create a Payout to [bank account](/api/x/payouts/create/bank-account) or [VPA](/api/x/payouts/create/vpa) using the Fund account id.
...
Know more about [payout states and life cycle](/x/payouts/states-life-cycle).
...
- We recommend subscribing to our [webhooks](/x/apis/subscribe) to get instant notifications. Webhooks are triggered whenever there is a change in the status of the payout. You can update your database using the webhook payload.

## Others Guidelines

### Fund Account Validation

Always validate your customer's Fund account to ensure it is the account number where your user wants to receive the amount. Know more about [Fund account validation](/api/x/account-validation). 
...
We recommend that you enable workflows on your account. This allows you to set up a maker-checker process. You can have 1 person create a payout and save it as a draft. Next, you can have someone from the financial team check the payouts before they are processed. Know more about [RazorpayX Maker-Checker](/x/manage-teams/approval-workflow).
...
This feature is enabled by default. You just have to consume the `payout.failed` event to utilise this feature. Know more about [Intelligent Payouts](/x/payouts/intelligent-payouts).

### Low Balance Alerts

We recommend that you set low balance alerts on your RazorpayX account. Once set, we will notify you when your RazorpayX account balance drops below the set threshold. Know more about [Low balance alerts](/x/account-types#low-balance-email-alerts).

### Queued Payouts

If your RazorpayX account does not have sufficient funds to process a payout, the payout fails.

To avoid such scenarios, we recommend passing `queue_if_low_balance: true` in the Payout API request. When you pass the parameter, if you do not have sufficient balance to process the payout, the payout is queued instead of failing. The queued payouts are automatically processed when you add funds to your RazorpayX account. Know more about [Queued Payouts](/x/payouts/queued).

### Handling 5XX Errors and Idempotency

We recommend you ensure your payout API requests are idempotent. Idempotency allows you to safely retry or send the same request multiple times without fear of performing the same action more than once. Know more about [RazorpayX Idempotency](/api/x/payout-idempotency/make-request).

<Warning>

**Watch Out!**

[Idempotency key](/api/x/payout-idempotency/make-request) is mandatory to make a successful payout.
...
Know more about [Handling 5XX Errors](/errors/x#handling-5xx-errors) and [Idempotency](/api/x/payout-idempotency/make-request).

</Warning>
### IP Allowlisting

It is mandatory to [get your IP address allowlisted](/x/dashboard/allowlist-ip). This means we will process the API calls made from the allowlisted IPs only.
