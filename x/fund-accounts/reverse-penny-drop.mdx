...
description: "Reverse Penny Drop is a way to verify a bank account or VPA (UPI id). In this method, the end user — the person whose account needs to be verified — makes a ₹1 payment from their UPI app to a Razorpay-designated account. Once Razorpay receives this payment, it automatically pulls the payer's verified account details directly from their bank. This includes their full name, bank account number, IFSC code, account type and VPA."
---
...
- [Reverse Penny Drop FAQs](/x/fund-accounts/faqs#reverse-penny-drop)
