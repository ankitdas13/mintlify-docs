...
description: "After you set up an [approval workflow](/x/manage-teams/approval-workflow) and [create bulk payout](/x/bulk-payouts#create-bulk-payout), you can review the bulk payout batch files and approve them with a single OTP."
---
...
1. Log in to the [RazorpayX Dashboard](https://x.razorpay.com/auth) as an [Owner/Admin](/x/manage-teams) and go to **My Account & Settings** under the user profile icon. 
...
You have successfully enabled the setting to approve a batch of bulk payout with a single OTP. Your team can now create bulk payout batches using the [new templates](/x/bulk-payouts#download-templates) and send them for approval.

## Review Bulk Payouts 

To approve/reject the bulk payout batches pending on you: 

1. Log in to the [RazorpayX Dashboard](https://x.razorpay.com/auth) as an [Owner/Approver](/x/manage-teams).
...
Only the bulk payout batch file moves to the `processing` status. Know more about [Bulk Payouts batch processing vs Payout processing](/x/bulk-payouts#bulk-payouts-batch-processing-vs-payouts-processing).
