...
description: "UPI payments enable customers to make payments using a Virtual Payment Address (VPA) without entering bank information."
---
...
In this flow, customers likely enter invalid VPAs or forget their VPAs, which could lead to higher drop-off rates. To overcome this problem, Razorpay enables you to validate and save the VPAs of a customer. Know more about [Saved VPA](/payments/payment-gateway/s2s-integration/payment-methods/upi/saved-vpa).
...
- [Validate the VPA](/payments/payment-gateway/web-integration/custom/features/validate-vpa) before initiating the payment request.
...
- Use the [Saved VPA](/payments/payment-gateway/s2s-integration/payment-methods/upi/saved-vpa) feature offered by Razorpay to provide a better customer experience.
...
<Steps>
<Step title="[Create a customer](#step-1-create-a-customer) or fetch the custome...">
[Create a customer](#step-1-create-a-customer) or fetch the customer for whom the VPA should be saved.
</Step>
<Step title="[Create an order](#step-2-create-an-order).">
[Create an order](#step-2-create-an-order).
</Step>
<Step title="[Validate the VPA](#step-3-validate-the-vpa) entered by the customer.">
[Validate the VPA](#step-3-validate-the-vpa) entered by the customer.
</Step>
<Step title="[Initiate payment](#step-4-initiate-a-payment) with a collect reque...">
[Initiate payment](#step-4-initiate-a-payment) with a collect request on the provided VPA.
</Step>
<Step title="[Handle Payment Success and Error Events](#step-5-handle-payment-su...">
[Handle Payment Success and Error Events](#step-5-handle-payment-success-and-error-events).
</Step>
</Steps>
...
UPI Collect is deprecated effective 28 February 2026. This tab is applicable only for exempted businesses. If you are not covered by the exemptions, refer to the [migration documentation](/announcements/upi-collect-migration/s2s-integration) to switch to UPI Intent.
...
: `json object` Key-value pairs that can hold additional information about the payment. 
 Refer to the [Notes](/api/understand#notes) section of the API Reference Guide.
...
If the payment has failed, the callback will contain details of the error. Refer to [Errors](/errors/payments/payment-methods-error-parameters#upi) for details.
...
<Steps>
<Step title="[Create an order](#step-1-create-an-order).">
[Create an order](#step-1-create-an-order).
</Step>
<Step title="[Fetch all tokens of a customer](#step-2-fetch-vpa-tokens-of-a-cust...">
[Fetch all tokens of a customer](#step-2-fetch-vpa-tokens-of-a-customer).
</Step>
<Step title="[Initiate a payment](#step-3-initiate-a-payment-using-vpa-token) wi...">
[Initiate a payment](#step-3-initiate-a-payment-using-vpa-token) with the token selected by the customer.
</Step>
<Step title="[Handle Payment Success and Error Events](#step-4-handle-payment-su...">
[Handle Payment Success and Error Events](#step-4-handle-payment-success-and-error-events).
</Step>
</Steps>
...
`currency` _mandatory_
: `string` ISO code for the currency in which you want to accept the payment. Refer the [list of supported currencies](/payments/international-payments#supported-currencies).
...
- Poll Razorpay servers periodically for the [payments made for the order](/api/orders/fetch-payments) using our Fetch Payment APIs.
...
- [UPI Error Codes](/errors/payments/payment-methods-error-parameters#upi)
