...
description: "The steps required to integrate tokens in the payment flow are as follows:"
---
...
-  Poll the Razorpay servers to [fetch a payment](/api/payments/fetch-with-id) or [fetch the payments made for an order](/api/payments#fetch-payments-based-on-orders).

- Subscribe to [payments](/webhooks/payments) and [orders](/webhooks/orders) Webhook events that are generated in Razorpay when the customer completes the payment on your website.
