...
description: "You can collect payments using the UPI intent flow that will be handled by UPI apps installed on your customers' mobile devices."
---
...
4. **Mobile site**  
  ...
  If you have higher traffic via mobile site, then make sure you provide the option of UPI intent payment to your users using our [m-site integration](/payments/payment-methods/upi/google-pay/custom-integration). This will help you achieve better success rates.
5. **Gpay SDK** 
...
<Step title="[Verify the payment status](#step-3-verify-the-payment-status).">
...
: `json object` A key-value pair that can hold additional information about the payment. 
 Refer the [Notes](/api/understand#notes) section of the API Reference Guide.
...
For the package name, you can check the [list of supported UPI apps](/payments/payment-methods/upi/supported-apps).
...
- Poll Razorpay servers periodically for the payments made for the order using our [Fetch Payment APIs](/api/orders/fetch-payments).

- [Subscribe to the Webhook events](/webhooks/setup-edit-payments) created in our system for each of the following entities:
...
[Error reasons](/errors#list-of-error-reasons-for-payments)
