...
description: "If you integrated with our S2S APIs before October 15, 2022, you must make the following changes to your integration to accept card payments with 3DS2 authentication protocol."
---
...
Ensure you make the following changes in your [Create a Payment API](/payments/payment-gateway/s2s-integration/redirect/build-integration/cards#12-create-a-payment) request. There is no change in the response.

Parameter Changes | Description
---
New Parameters | Pass these new parameters: - `authentication` and related child parameter: These determine the [authentication channel](/payments/payment-gateway/s2s-integration/redirect/build-integration/cards/migrate-3ds2.0#:~:text=Details%20of%20the-,authentication%20channel,-.) being used.
- `browser` and related child parameters:  These capture the customer's [browser details](/payments/payment-gateway/s2s-integration/redirect/build-integration/cards/migrate-3ds2.0#:~:text=regarding%20the%20customer%27s-,browser,-.%20This%20parameter%20need), which are sent to the banks to aid their risk analysis.
...
Descriptions for the response parameters are present in the [Payments Entity](/api/payments#payments-entity) parameters table.
...
The rest of the integration steps mentioned in the [S2S Redirect Cards Build Integration document](/payments/payment-gateway/s2s-integration/redirect/build-integration/cards) remain the same. No changes are required in those.

After completing the build integration steps, you can continue with [Step 2: Test Integration](/payments/payment-gateway/s2s-integration/redirect/test-integration)
