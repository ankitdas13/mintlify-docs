...
description: "Third-party validation (TPV) of bank accounts is mandatory for businesses in the BFSI (Banking, Financial Services and Insurance) sector dealing with Securities, Broking and Mutual Funds. You can accept customer payments by integrating with the Turbo UPI - TPV SDK."
---
...
### Prerequisites
         ...
         2. Integrate with [Razorpay Flutter Standard Integration](/payments/payment-gateway/flutter-integration/standard/integration-steps).
...
- Use the `rzp_test_0wFRWIZnH65uny` API key id for testing on the UAT environment and the [Razorpay live keys](/api/authentication#live-mode-api-keys) for prod testing.
...
`notes` _optional_
: `object` This is a key-value pair that can be used to store additional information about the entity. It can hold a maximum of 15 key-value pairs, 256 characters (maximum) each. For example, `"note_key": "Beam me up Scottyâ€`.
...
`notes`
: `json object` This is a key-value pair that can be used to store additional information about the entity. It can hold a maximum of 15 key-value pairs, 256 characters (maximum) each. For example, `"note_key": "Beam me up Scottyâ€`.
...
### Step 2: Create an Order *(Mandatory)*
         ...
         ```
                ...
                Request Parameters
                 ...
                 `amount` _mandatory_
: `integer` The transaction amount expressed in paise (currency supported is INR). For example, for an actual amount of â‚¹1, the value of this field should be `100`.
...
`amount`
: `integer` The order amount represented in the smallest unit of the currency passed. For example, amount = 100 translates to 100 paise, that is â‚¹1 (default currency is INR).
...
`notes`
: `object` Key-value pair you can use to store additional information about the entity. Maximum of 15 key-value pairs, 256 characters each. For example, "note_key": "Beam me up Scottyâ€.
...
### 3.3 Submit Method
                     ...
                     To accept payments, call Custom Checkoutâ€™s `submit` method with the following payload:
...
### Step 4: Handle Payment Success and Failure
         ...
         The way you handle payment success and failure scenarios depends on the Checkout sample code you opted for in the previous step. Know how to [handle payment success and failure](/payments/third-party-validation/standard-integration#step-14-handle-payment-success-and-failure).
...
- `SERVER_ERROR`: Failure at PSP.
         ...
         [Refer to the list of possible error reasons](/payments/payment-gateway/flutter-integration/standard/payment-methods/turbo-upi/error-codes).
...
- Import the prod library from the GitHub repository â†’ `https://github.com/upi-turbo/android-turbo-sample-app/tree/prod/app/libs` prod branch.
...
- Replace the UAT credential with the [Razorpay live keys](/api/authentication#live-mode-api-keys) for prod testing.
