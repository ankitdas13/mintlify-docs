---
title: "About Errors"
description: "All successful responses are returned with HTTP Status code 200. In case of failure, Razorpay API returns a JSON error response with the parameters that detail the reason for the failure."
---
...
If you are using an official [Razorpay Language-wise SDK Integration](/payments/server-integration), the error responses result in exceptions that need to be handled in your integration.
...
To understand more about error codes, refer to the [Error Codes](/errors) section.
