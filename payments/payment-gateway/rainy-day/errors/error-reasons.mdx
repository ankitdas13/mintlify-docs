---
title: "Error Reasons"
description: "All the possible values for the `reason` parameter in the error response along with their explanation and the next best action are shown below."
---
...
---
`collect_on_mcc_blocked` | NPCI blocks collect requests on certain [MCCs](/payments/payment-methods/upi/google-pay#android). | Please retry using intent flow.
---
...
To understand error codes specific for each payment method supported by Razorpay, refer to the payment flows described in the [Payment Method Error Parameters](/errors/payments/payment-methods-error-parameters) section.
