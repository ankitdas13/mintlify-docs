...
description: "To set up webhooks:"
---
...
You can set up to **30 URLs** to receive Webhook notifications. Webhooks can only be delivered to public URLs. If you attempt to save a localhost endpoint as part of a webhook setup, you will notice an error. Know more about [testing Webhooks on an application running on localhost](/webhooks/validate-test#application-running-on-localhost).

    
            2. Enter a **Secret** for the webhook endpoint. The secret is used to validate that the webhook is from Razorpay. Do not expose the secret publicly. Know more about [how to validate webhooks](/webhooks/validate-test).
...
### 9. I am getting the following error: "Plugin could not be activated because it triggered a fatal error.
         ...
         2. Remove the WooCommerce webhook associated with Razorpay from the Dashboard. Know how to [delete a webhook](/webhooks/setup-edit-payments#delete-a-webhook).
...
### 11. What troubleshooting procedures should be carried out before initiating a support ticket?
         ...
         1. Ensure that your system meets all the requirements mentioned [here](/payments/payment-gateway/ecommerce-plugins/woocommerce#compatibilities-and-dependencies).
...
### 21. Why is the payment captured on Razorpay but not updated on WooCommerce for some orders?
         ...
         1. Ensure that the [webhook is configured correctly](/payments/payment-gateway/ecommerce-plugins/woocommerce/troubleshooting-faqs#2-my-webhooks-are-not-auto-configured-since-i) in your Razorpay Dashboard.
         2. After placing an order, check the [WooCommerce site logs](/payments/payment-gateway/ecommerce-plugins/woocommerce/troubleshooting-faqs#20-how-and-where-can-i-check-the) for entries such as:
         ...
         If these logs are missing, the issue might occur due to domain allowlisting. To resolve this, list [Razorpay's IPs](/security/whitelists#webhook-ips) in your server settings.
