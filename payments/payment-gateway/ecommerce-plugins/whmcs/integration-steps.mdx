...
description: "Follow the steps given below to integrate Razorpay Payment Gateway with your WHMCS website."
---
...
### 1.2 Configure Webhooks
            ...
            5. Copy the URL that appears on the screen. In the Razorpay Dashboard, configure this URL in **Accounts & Settings** → **Webhooks**. Know more about how to [setup webhooks](/webhooks).
            6. Enter the secret.
...
Webhooks can only be delivered to public URLs. You will notice an error if you attempt to save a localhost endpoint as part of a webhook setup. Know about [validating and testing webhooks for alternatives to localhost](/webhooks/validate-test#test-webhooks).

                     - Enter the Secret you provided on the WHMCS site. The secret is used to validate where the webhook is from Razorpay. Do not expose the secret publicly. Know about how to [validate webhooks](/webhooks/validate-test#validate-webhooks).
