...
description: "Follow the steps given below to integrate Razorpay Payment Gateway with your Magento website."
---
...
You should see `Razorpay_Magento` in the status. It might appear on the disabled modules list.
                     ...
                     **Upgrade Magento Extension**
                     ...
                     If you are an existing extension user, you can [upgrade](/payments/payment-gateway/ecommerce-plugins/magento#upgrade-magento-extension) to the latest version using the composer.
...
### Step 2: Configure Magento Store 2.x
                 ...
                 5. Enter your test mode [KEY_ID] and [KEY_SECRET]. These can be [generated from the Razorpay Dashboard](/api/authentication#generate-api-keys).
...
- Webhook is auto-configured on Magento plugin version 3.8.1-beta and above. For versions lower than 3.8.1-beta, you should [configure webhooks manually](/payments/payment-gateway/ecommerce-plugins/magento/integration-steps#step-3-set-up-webhooks).
...
Webhooks can only be delivered to public URLs. If you attempt to save a localhost endpoint as part of a webhook set-up, you will notice an error. Please refer to the [test webhooks](/webhooks/validate-test) section for alternatives to localhost.

                    - Enter the Secret you had provided on the Magento site. The secret is used to validate that the webhook is from Razorpay. Do not expose the secret publicly. Know more about [webhooks validation](/webhooks/validate-test#validate-webhooks).
