...
description: "Follow the [build integration](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps#1-build-integration) steps to integrate 1 Razorpay with your Shopify store."
---
...
### 1. How do I integrate 1 Razorpay with my Shopify store?
         ...
         Follow the [build integration](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps#1-build-integration) steps to integrate 1 Razorpay with your Shopify store.
...
### 4. Why is the Activate button disabled?
         ...
         - **Check for Integration with Another Account:** Verify if you have integrated your Shopify Store with a different Razorpay MID. If yes, revoke access under applications and retry the 1 Razorpay [integration steps](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps#1-build-integration). 
...
### 5. I am getting the following error message, "Email ID Mismatch." Why?
         ...
         You can [update the email ID](/payments/dashboard/account-settings/account-details#update-login-email) on Razorpay to match the one used in Shopify. Verify that the email ID associated with owner access on Razorpay matches the one used in Shopify.
...
### 8. What should I do if I am confused between legacy and current plugin versions?
         ...
         To avoid confusion, uninstall the legacy version of the plugin and [re-install](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps#1-build-integration) the current plugin version (1 Razorpay).
        

    
### 9. Our Payment Gateway stopped displaying after migrating to Shopify. How can we reactivate it?

         After migrating your website to Shopify, you need to integrate with the official Shopify 1 Razorpay plugin, as your previous Payment Gateway integration from the old platform will not be carried over. Follow the [integration steps](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps) and once activated, your payment gateway will be displayed at checkout, allowing you to accept payments.
...
### 11. How do I upgrade to 1 Razorpay?
         ...
         You can upgrade to 1 Razorpay by following a simple migration process. Know more about [how to migrate to 1 Razorpay](/payments/payment-gateway/ecommerce-plugins/shopify/migration-steps).
...
### 16. My customer is getting the following error when they make payments on my Shopify store, "There was an issue processing your payment. Try again or use a different payment method." What should I do?
         ...
         Follow the [build integration](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps#1-build-integration) steps to install the plugin again. 
...
### 19. How can I test a payment for 1 Razorpay on the Shopify store?
         ...
         You can test a payment for 1 Razorpay on the Shopify store by switching to test mode. Know more about [how to test a transaction in test mode.](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps#21-make-a-test-transaction-in-test-mode)
...
### 21. How can I enable automated payment receipts with GST split?
         ...
         Razorpay facilitates only payment collection. The partner is solely responsible for sending invoices or payment receipts with GST details to customers. To provide GST-compliant receipts, partners can use [Razorpay Invoice](/payments/invoices) product when collecting payments.
...
### 23. Can I integrate a second Razorpay account with my Shopify store?
         ...
         No, Shopify currently supports only one Razorpay account integration per store. If you want to integrate a different Razorpay account, you need to [deactivate](#7-how-do-i-uninstall-razorpay-payment-apps) the existing integration first and then [activate](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps) the new account.
...
### 29. How do I integrate Meta Pixel to accept payments through Meta Ads?
         ...
         Razorpay does not provide a direct plugin integration for accepting payments through Meta Ads. However, you can use Razorpay Payment Pages with [Facebook Pixel and Google Tracking ID](/payments/payment-pages/plugins-add-ons) to track payments and conversions from your Ad campaigns.
        

    
### 30. Can I use Razorpay Route API with my Shopify store?

         No, Razorpay Route API is not supported for Shopify integration. Only the [1 Razorpay plugin](/payments/payment-gateway/ecommerce-plugins/shopify/integration-steps) is supported for Shopify stores.
