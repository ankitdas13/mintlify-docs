...
description: "Follow the steps given below to integrate Razorpay Payment Gateway with your CS-Cart website."
---
...
Webhook is auto-configured when you enter and save the API key ID and secret on the plugin settings page. You need to verify that webhooks are enabled on your Razorpay [Dashboard](/payments/payment-gateway/ecommerce-plugins/cs-cart/troubleshooting-faqs#2-how-can-i-verify-if-webhooks-are). However, for versions lower than 1.4.0, you need to [manually configure webhooks](/payments/payment-gateway/ecommerce-plugins/cs-cart/troubleshooting-faqs#1-my-webhooks-are-not-auto-configured-since-i).
