...
description: "With Razorpay Turbo UPI, businesses experience faster and simpler payments. It condenses the payment process from 5 steps to just 1, eliminating app redirections. Enjoy a seamless in-app payment experience, reduce dependencies on third-party UPI apps, and gain complete visibility of the payment journey."
---

You can seamlessly integrate Turbo UPI with Razorpay iOS Standard SDK. Explore the full potential of [Razorpay Turbo UPI](/payments/payment-methods/upi/turbo-upi) and know How it Works.

## Prerequisites

- Integrate with [Razorpay iOS Standard SDK](/payments/payment-gateway/ios-integration/standard/integration-steps). Ensure that you integrate with SDK version 1.0.0 or higher.
...
Ensure your customers [onboard with Razorpay Turbo UPI](/payments/payment-methods/upi/turbo-upi#2-onboarding-flow) to get started.
...
Razorpay SDK will handle all the changes related to `UpiTurbo` internally. To integrate with the payment flow, [pass payment option and display the checkout form](/payments/payment-gateway/ios-integration/standard/integration-steps#14-pass-payment-options-and-display-checkout-form).
...
- `internal`
         ...
         [Refer to the list of possible error reasons](/payments/payment-gateway/ios-integration/standard/payment-methods/turbo-upi/error-codes).
