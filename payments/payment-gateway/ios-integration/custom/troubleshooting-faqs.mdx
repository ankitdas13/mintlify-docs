...
description: "When a framework is distributed, it contains architectures of the simulators so that the consumer of the framework can build it on a simulator. However, when uploading the archive on iTunes, you have to strip these archi"
---
...
<Step title="Step 2">
In the project navigator, locate the `Podfile` in the root directory of your project.
</Step>
<Step title="Step 3">
Open the `Podfile` and look for the line that specifies the Razorpay SDK pod. The version number is represented by `x.y.z`.
</Step>
</Steps>
...
<Steps>
<Step title="Step 1">
In your project’s Podfile, specify the [latest SDK version](https://github.com/razorpay/razorpay-customui-pod/releases/). If you do not mention any version, it automatically picks the latest version from Cocoapods.
</Step>
<Step title="Step 2">
Run `pod repo update` to fetch the latest release versions of the pods.
</Step>
<Step title="Step 3">
After updating, ensure that the integration is successful and there are no issues with the updated SDK.
</Step>
</Steps>
