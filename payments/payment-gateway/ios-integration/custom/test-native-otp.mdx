...
description: "Integrate the [Razorpay Native OTP](/payments/payment-gateway/ios-integration/custom/native-otp) feature with iOS custom checkout to avoid customer payment issues such as payment failures due to low internet speeds and bank page redirects. After the integration is complete, you need to test the integration to ensure that it is working as expected. You can make a test transaction using the test cards, verify the payment status from the Dashboard, APIs or subscribe to related Webhook events to take appropriate actions at your end. After testing the integration in test mode, you can start accepting actual payments from your customers."
---
...
Know [how to verify payment status](/payments/payment-gateway/ios-integration/custom/build-integration#110-verify-payment-status).
...
Know more about how to [Accept Live Payments](/payments/payment-gateway/ios-integration/custom/go-live-checklist#accept-live-payments).
