...
description: "Razorpay payment gateway supports one-time passwords (OTPs) in the Checkout itself, preventing the customers from being redirected to the ACS page of their respective issuing banks."
---
...
2. Integrate with the [Razorpay iOS Custom SDK](/payments/payment-gateway/ios-integration/custom).
...
To reuse the Razorpay Checkout web integration inside a WebView on Android or iOS, pass a [callback_url](/payments/payment-gateway/callback-url) along with other checkout options to process the desired payment.
