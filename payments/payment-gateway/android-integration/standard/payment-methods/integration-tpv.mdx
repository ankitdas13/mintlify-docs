...
description: "Third-party validation (TPV) of bank accounts is mandatory for businesses in the BFSI (Banking, Financial Services and Insurance) sector dealing with Securities, Broking and Mutual Funds. You can accept customer payments with the Turbo UPI TPV SDK. Know more about [How TPV works](/payments/third-party-validation#how-it-works)."
---
...
You can choose to maintain a whitelisted account per customer using the S2S [Customer Add Bank Account API](/api/customers/bank-accounts#1-add-bank-account-of-customer) on your end. This is an additional functionality. This will help you during the onboarding flow, where only whitelisted accounts will be shown to the users during onboarding.
...
### Prerequisites
         ...
         2. Integrate with the [Razorpay Android Standard Checkout SDK](/payments/payment-gateway/android-integration/standard/integration-steps).
...
- Use the [Razorpay live keys](/api/authentication#live-mode-api-keys) for prod testing.
...
### Step 4: Payment Flow
         ...
         Razorpay SDK will handle all the changes related to Turbo UPI internally. To integrate with the payment flow, [initiate payment and display the checkout form](/payments/payment-gateway/android-integration/standard/integration-steps#14-initiate-payment-and-display-checkout-form).
...
- Use the [Delete Bank Account of Customers API](/api/customers/bank-accounts#2-delete-bank-account-of-customer) to delete bank account of the customers.
...
- `SERVER_ERROR`: Failure at PSP.
         ...
         [Refer to the list of possible error reasons](/payments/payment-gateway/android-integration/standard/payment-methods/turbo-upi/error-codes).
