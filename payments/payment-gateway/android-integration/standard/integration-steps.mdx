...
description: "Follow these steps to integrate your Android application:"
---
...
### 1.2 Initialize Razorpay Android Standard SDK
     ...
     Add your `` dynamically using Checkout's `setKeyId()` method. You can generate the [API Keys](/payments/dashboard/account-settings/api-keys#generate-api-keys) from the Dashboard.
...
### JSONObject
         ...
         Create an instance of the `Checkout` and pass the payment details and options as a `JSONObject`. Ensure that you add the `order_id` generated in [Step 3](/payments/payment-gateway/android-integration/standard/integration-steps#13-create-an-order-in-server).
