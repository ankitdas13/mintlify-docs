...
description: "Follow these steps to integrate your Android application with Razorpay Android Custom SDK:"
---
...
Add your [API keys](/api/authentication#generate-api-keys) to `AndroidManifest.xml`.
...
The auto-OTP reading feature is available only for saved cards. Know more about [saved cards](/payments/dashboard/account-settings/checkout-features#flash-checkout).
...
: `string` The currency in which the transaction should be made.  See the [list of supported currencies](/payments/international-payments#supported-currencies). Length must be of 3 characters.
...
Check the various [payment methods](/payments/payment-gateway/android-integration/custom/payment-methods) available with Razorpay Android Custom SDK.

<Info>

**Handy Tips**

If you are using Subscriptions, you can pass the `subscription_id` in the options, which fetches subscription-related details along with the payment method. This saves you a network call to get amount for that `subscription_id`. Know more about [Subscriptions](/payments/subscriptions).
...
`key` _mandatory_
: `string` API Key ID generated from **Dashboard** → **Account & Settings** → [API Keys](/payments/dashboard/account-settings/api-keys#generate-api-keys).
...
To reuse the Razorpay Checkout web integration inside a web view on Android or iOS, pass a [callback_url](/payments/payment-gateway/callback-url) along with other checkout options to process the desired payment.
...
[Step 2: Test Integration](/payments/payment-gateway/android-integration/custom/test-integration)

### Related Information
- [Integrate With Android Custom SDK](/payments/payment-gateway/android-integration/custom)
- [Payment Methods](/payments/payment-gateway/android-integration/custom/payment-methods)
- [Troubleshooting & FAQs](/payments/payment-gateway/android-integration/custom/troubleshooting-faqs)

- [Additional Support for Payment Methods](/payments/payment-gateway/android-integration/custom/additional-features)
