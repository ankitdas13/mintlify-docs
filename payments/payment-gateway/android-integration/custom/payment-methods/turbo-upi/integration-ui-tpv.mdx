...
description: "Third-party validation (TPV) of bank accounts is mandatory for businesses in the BFSI (Banking, Financial Services and Insurance) sector dealing with Securities, Broking and Mutual Funds. You can accept customer payments with the Turbo UPI UI TPV SDK. Know more about [How TPV works](/payments/third-party-validation#how-it-works)."
---
...
### Prerequisites
         ...
         2. Integrate with the [Razorpay Android Custom SDK](/payments/payment-gateway/android-integration/custom/build-integration).
...
- Use the `rzp_test_5sHeuuremkiApj` API key id for testing on the UAT environment and the [Razorpay live keys](/api/authentication#live-mode-api-keys) for prod testing.
...
If the order is not created, you can provide CustomerId to get bank accounts whitelisted for the customer.
                 ...
                 ```
                        ...
                        Request Parameters
                         ...
                         ---
                         `setCustomerID` | The unique identifier of the customer. You can create `customer_id` using the [Customers API](/api/customers).
                         ----
                         `setActivity` | Sets the context or activity for the operation.
                         ---
                         `setOrderID` | Order ID generated via [Orders API](/payments/third-party-validation#upi-only).
                         ----
...
- `SERVER_ERROR`: Failure at PSP.
         ...
         [Refer to the list of possible error reasons](/payments/payment-gateway/android-integration/custom/payment-methods/turbo-upi/error-codes).
