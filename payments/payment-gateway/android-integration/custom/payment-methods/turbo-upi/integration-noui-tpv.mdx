...
description: "Third-party validation (TPV) of bank accounts is mandatory for businesses in the BFSI (Banking, Financial Services and Insurance) sector dealing with Securities, Broking and Mutual Funds. You can accept customer payments by integrating with the Turbo UPI Headless - TPV SDK."
---
...
### Prerequisites
         ...
         2. Integrate with [Razorpay Android Custom SDK](/payments/payment-gateway/android-integration/custom/build-integration).
...
`customer_id`
: The unique identifier of the customer. You can create `customer_id` using the [Customers API](/api/customers).
...
: Order ID generated via [Orders API](/payments/third-party-validation#upi-only).
