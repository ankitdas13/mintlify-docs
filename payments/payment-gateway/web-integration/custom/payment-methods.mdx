...
description: "The Razorpay Web Custom SDK lets you integrate the supported payment methods on your website's checkout form."
---

## Fetch Payment Methods

Use the [Fetch Payment Methods API](/payments/payment-gateway/android-integration/custom/build-integration#14-fetch-payment-methods) to fetch the payment methods available for your account.
...
There are no specific request parameters to be passed. Instead, you must pass the `fetchVirtualAccount` method for your Customer Identifier to get created and the details to appear on the checkout. Know more about [integrating bank transfer with Custom Checkout](/payments/payment-methods/bank-transfer/custom-integration).
...
If you want to securely store the customer's card details as network tokens, know about [Saved Cards feature](/payments/payment-gateway/web-integration/custom/features/saved-cards).
...
You can list the available banks using a drop-down for customers. You can obtain a list of banks using the [Fetch Supported Methods API](/payments/payment-gateway/web-integration/custom/build-integration#12-fetch-payment-methods).
...
- `paypal` (requires [approval](https://razorpay.com/support/#request) and [onboarding](/payments/payment-methods/wallets/paypal))
- `phonepeswitch` (requires [approval](https://razorpay.com/support/#request))

#### PhonePe Switch

You can accept in-app payments from your customers transacting in PhonePe Switch. Know more about [PhonePe Switch Integration](/payments/payment-gateway/web-integration/custom/phonepe-switch).
...
You can avail the UPI Intent flow by integrating with the [Google Pay SDK](/payments/payment-methods/upi/google-pay/custom-integration).
...
Know how to integrate with [UPI Intent on Mobile Web](/payments/payment-gateway/web-integration/custom/payment-methods/upi-intent-mweb).
...
You can accept UPI payments using the collect flow. Know more about [Validate VPA](/payments/payment-gateway/web-integration/custom/features/validate-vpa) and [Saved VPA](/payments/payment-gateway/web-integration/custom/features/saved-vpa).
...
You can accept recurring payments from your customers using `emandate`, `card` or `upi` as the method. For more information about authorisation and subsequent payments, refer to the [Recurring Payments documentation](/payments/recurring-payments).
...
Know more about [Recurring Payments APIs](/api/payments/recurring-payments/custom).
...
- Pass the `method` and `provider` parameters in [Create Payment Method](/payments/payment-gateway/web-integration/custom/build-integration#133-submit-payment-details).
