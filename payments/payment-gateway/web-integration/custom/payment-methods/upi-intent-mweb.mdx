...
description: "Integrate UPI Intent on your mobile app. UPI Intent works for UPI PSP apps and is available on Android and iOS."
---
...
2. Instantiate Razorpay with your Key ID generated from the [Dashboard](/api/authentication#generate-api-keys). Skip this step if you have already completed it.
...
<Steps>
<Step title="Only activate the UPI payment intent when a customer initiates it r...">
Only activate the UPI payment intent when a customer initiates it rather than when the page loads.
</Step>
<Step title="Create a payment immediately after the customer clicks the pay butt...">
...
<Step title="When a customer chooses a UPI payment method, pass the value into `...">
When a customer chooses a UPI payment method, pass the value into `razorpay.createPayment(paymentData, {app: 'phonepe'})`.
</Step>
<Step title="Display a loading screen after starting the UPI payment intent flow...">
