---
title: "About Razorpay Trusted Business Widget"
description: "Razorpay Trusted Business (RTB) Widget offers a simple solution to enhance trust and reduce drop-offs. You can seamlessly embed the widget on your website with two integration steps."
---
...
- Generate the API Keys on the Dashboard. You can use the  [Test Mode keys](/api/authentication#live-mode-api-keys) to test the integration and preview the Widget. Later, switch to **Live Mode** on the Dashboard, generate the [Live API keys](/api/authentication#live-mode-api-keys) and replace it with the test keys.
- Check if you are [eligible](/payments/payment-gateway/features/trusted-business#eligibility-requirements) for Razorpay Trusted Business on the Dashboard. 
...
- Currently, the RTB widget is supported on Web and [shopify](/payments/widgets/trusted-business/rtb-widget-shopify).
...
### Step 2: Add Widget on Website
         ...
         Create a custom Razorpay Trusted Business widget HTML element based on where you want to place the widget and link the following code to your HTML file. Replace the key with your Test Key generated from the Dashboard.
            ...
            `key` _mandatory_
            : `string` API Key id generated from the [Dashboard](/api/authentication#generate-api-keys). For example, `rzp_test_XXXX00000XXXX`.
...
After you preview and test the widget on your Dashboard, switch to the live mode to generate live API keys. Replace the test keys with these live keys to take the integration live. Know more about [live API keys](/api/authentication#live-mode-api-keys).
