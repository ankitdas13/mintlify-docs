---
title: "Integrate Magic Checkout With WooCommerce Website"
description: "Follow the steps given below to integrate Razorpay Magic Checkout with your WooCommerce Website."
---
...
If you are integrating with our plugin for the first time, refer to the [FAQs](/payments/magic-checkout/troubleshooting-faqs#woocommerce).
...
- Webhooks are auto-configured when you enter and save the API key ID and secret on the plugin settings page. You need to verify if webhooks are enabled on your [Razorpay Dashboard](/payments/magic-checkout/troubleshooting-faqs#4-how-can-i-verify-if-webhooks-are). However, for versions lower than 3.5.0, you need to [configure webhooks manually](/payments/magic-checkout/troubleshooting-faqs#3-my-webhooks-are-not-auto-configured-since-i).
...
To disable Magic Checkout, refer to our [FAQs](/payments/magic-checkout/troubleshooting-faqs#5-can-i-disable-magic-checkout).
...
- [Cash on Delivery](/payments/magic-checkout/woocommerce/configuration#cash-on-delivery): Enable COD and configure rules for specific locations, products, and order amounts, catering to customer preferences and logistical needs to increase sales.
- [RTO](/payments/magic-checkout/woocommerce/configuration#rto): Use automated COD intelligence or manual review to decide whether to offer COD based on customer buying history, thus reducing RTO rates.
- [Convert COD Orders to Prepaid](/payments/magic-checkout/woocommerce/configuration#convert-cod-orders-to-prepaid): Encourage COD customers to switch to prepaid by offering discounts or incentives, reducing cash handling risks.
- [Shipping Options](/payments/magic-checkout/woocommerce/configuration#shipping-options): Set up shipping rates at the product, zone and method levels to optimise costs and improve delivery efficiency.
- [International Shipping](/payments/magic-checkout/woocommerce/configuration#international-shipping): Allow customers to enter international PIN codes, expanding your market reach globally.
- [Coupons](/payments/magic-checkout/woocommerce/configuration#coupons): Provide discounts via coupons to enhance engagement and reduce cart abandonment.
- [Google and Facebook Analytics](/payments/magic-checkout/woocommerce/configuration#google-and-facebook-analytics): Integrate Google and Facebook Analytics for insights into customer behaviour and optimised marketing strategies.
