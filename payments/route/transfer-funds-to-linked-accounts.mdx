...
description: "You can transfer funds to your Linked Accounts using the following methods:"
---
...
<Steps>
<Step title="Step 1">
Create Linked Accounts. **Linked Accounts have a cooling period of 24 hours after which you can initiate transfers.**
</Step>
<Step title="Step 2">
Provide [Dashboard](/payments/route/linked-account) access to the Linked Account to ensure transparency between the two parties, such as:
...
<Step title="Step 3">
Ensure the Direct Transfer feature is enabled for your account. Contact our [support team](https://razorpay.com/support/#request) for more information.
</Step>
</Steps>
...
This eliminates the extra step of triggering the [Direct Transfers](/api/payments/route/direct-transfers) or [Transfers via Payments API](/api/payments/route/create-transfers-payments).
...
<Steps>
<Step title="Step 1">
Create an Order using the [Transfer via Orders API](/api/payments/route/create-transfers-orders).
</Step>
<Step title="Step 2">
[Capture the Payment](/payments/payment-gateway/how-it-works#capture-payments).
</Step>
<Step title="Step 3">
Set up the [webhook event `transfer.processed`](/webhooks/setup-edit-payments).
</Step>
</Steps>
...
You can initiate a transfer from a payment received from a customer. This can be done from the Dashboard or using the [Create Transfers from Payments API](/api/payments/route/create-transfers-payments).
...
<Step title="Step 2">
Under the **Payments** tab, click the relevant **Payment ID**.
</Step>
<Step title="Step 3">
On the Payment details pane,
...
<Step title="Step 4">
Add internal notes relevant to the transfer if any. You can choose to share the note with your Linked Account.
</Step>
<Step title="Step 5">
...
You can transfer funds to your Linked Accounts directly from your account balance using Direct Transfers. You can make direct transfers from the Dashboard or using the [Direct Transfer API](/api/payments/route/direct-transfers).
...
<Step title="Step 2">
Under the **Transfers** tab, click **+Create Direct Transfer**.

</Step>
<Step title="Step 3">
On the **Create Direct Transfer** pop-up page, provide the following details:
