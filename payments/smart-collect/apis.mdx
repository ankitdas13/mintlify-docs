...
Use [Smart Collect 2.0](/payments/smart-collect)to collect payments using UPI and other banking modes.
...
### List of Smart Collect 2.0 APIs
         ...
         Refer to the list of Smart Collect 2.0 APIs to create and manage Customer Identifiers of the type `VPA (UPI)`. 
            ...
            API | Description
            ---
            [Create a Customer Identifier With VPA Receiver](/api/payments/smart-collect-2/create-cust-id-vpa) | API to create a Customer Identifier with VPA receiver.
            ---
            [Create a Customer Identifier With VPA and Bank Account Receivers](/api/payments/smart-collect-2/create-cust-id-bank-account-vpa) | API to create a Customer Identifier with VPA and bank account receivers.
            ---
            [Fetch Payments Using UPI](/api/payments/smart-collect-2/fetch-payments-upi) | API to retrieve details of a UPI payment using the Payment id.
            ---
            [Add Receiver to an Existing Customer Identifier With VPA](/api/payments/smart-collect-2/add-receiver-vpa) | API to add receiver to an existing Customer Identifier. 
            ---
            [Add Receiver to an Existing Customer Identifier With Bank Account](/api/payments/smart-collect-2/add-receiver-bank-transfer) | API to add receiver to an existing Customer Identifier. 
...
### List of Smart Collect 2.0 API for TPV
         ...
         Refer to the list of Smart Collect 2.0 APIs to create and manage Customer Identifiers of the type `VPA (UPI)`with TPV.
            ...
            API | Description
            ---
            [Add VPA Receiver to an Existing Customer Identifier](/api/payments/smart-collect-2/tpv-add-receiver-vpa) | API to add receiver to an existing Customer Identifier. 
            ---
            [Add Bank Account Receiver to an Existing Customer Identifier](/api/payments/smart-collect-2/tpv-add-receiver-bank-transfer) | API to add receiver to an existing Customer Identifier. 
            ---
...
### List of Smart Collect APIs
            ...
            API | Description
            ---
            [Create Customer Identifier With Bank Account Receiver](/api/payments/smart-collect/create-cust-id-bank-account) | API to create a Customer Identifier with bank account receiver.
            ---
            [Fetch a Customer Identifier With ID](/api/payments/smart-collect/fetch-with-id) | API to Fetch a Customer Identifier by ID.
            ---
            [Fetch all Customer Identifiers](/api/payments/smart-collect/fetch-all) | API to fetch all Customer Identifiers.
            ---
            [Fetch Payments for a Customer Identifier](/api/payments/smart-collect/fetch-payments-cust-id)| API to fetch payments made against a particular Customer Identifier.
            ---
            [Fetch Payment Details using ID and Transfer Method](/api/payments/smart-collect/fetch-payments-bank-transfer) | API to retrieve details of a payment using the Payment ID and transfer method.
            ---
            [Fetch Payments Using Bank Transfer via UTR](/api/payments/smart-collect/fetch-payments-bank-transfer-utr) | API to retrieve details of a payment using bank transfer method via UTR.
            ---
            [Update a Customer Identifier](/api/payments/smart-collect/update) | API to update a Customer Identifier.
            ---
            [Close a Customer Identifier](/api/payments/smart-collect/close) | API to close a Customer Identifier.
...
### API-wise Integration Checklist for Smart Collect
            ...
            - Use [Create a Customer API](/api/customers#create-a-customer) if a Customer Identifier/UPI ID will be mapped to a unique customer.
            ...
            - We request you to create a single `customer_id` rather than making multiple IDs for the same customer. If their details change, you can use the [Edit Customer Details](/api/customers#edit-customer-details) API. Duplicate validation is done based on the combination of email ID and phone number.
            ...
            - Payments made to the Customer Identifiers must be within the [transaction limits](/payments/payment-methods/transaction-limits).
            - We recommend that you use [Fetch APIs](/api/payments/smart-collect#fetch-a-virtual-account-by-id) to make a GET call for any downstream dependencies. Webhooks are just a recommended layer on top of this.
            - We recommend you [Close the Customer Identifier](/api/payments/smart-collect#close-a-customer-identifier) once the payment is received.
...
### List of APIs for Smart Collect TPV
         ...
         To know more about Third Party Validation (TPV), click [here](/payments/smart-collect/third-party-validation).
            
            API | Description
            ---
            [Create Customer Identifier](/api/payments/smart-collect-tpv/create) | API to create a Customer Identifier.
            ---
            [Add an Allowed Payer Account](/api/payments/smart-collect-tpv/add-allowed-payer) | API to add allowed payers account details to a Customer Identifier.
            ---
            [Fetch a Customer Identifier by ID](/api/payments/smart-collect-tpv/fetch-with-id) | API to Fetch a Customer Identifier by ID.
            ---
            [Fetch all Customer Identifiers](/api/payments/smart-collect-tpv/fetch-all) | API to fetch all Customer Identifiers.
            ---
            [Fetch Payments for a Customer Identifier](/api/payments/smart-collect-tpv/fetch-payments)| API to fetch payments made against a particular Customer Identifier.
            ---
            [Fetch Payment Details using ID and Transfer Method](/api/payments/smart-collect-tpv/fetch-payment-id-transfer) | API to retrieve details of a payment using the Payment ID and transfer method.
            ---
            [Delete an Allowed Payer Account](/api/payments/smart-collect-tpv/delete-allowed-payer) | API to delete allowed payers account details added to a Customer Identifier.
            ---
            [Close a Customer Identifier](/api/payments/smart-collect-tpv#close-a-virtual-account) | API to close a Customer Identifier.
...
### API-wise Integration Checklist for Smart Collect TPV
            ...
            - Use [Create a Customer API](/api/customers#create-a-customer) if a Customer Identifier is mapped to a unique customer.
            - You can use the **fail_existing** (set to `"1"`) API parameter to create a customer. This helps you avoid `customer_id` being created multiple times for the same customer and will help in your reconciliation process.
            - We request you to create a single `customer_id` rather than making multiple IDs for the same customer. If their details change, you can use the [Edit Customer Details](/api/customers#edit-customer-details) API. Duplicate validation is done based on the combination of email ID and phone number.
            ...
            - The allowed payer can be deleted or added later using the [Add an Allowed Payer](/api/payments/smart-collect-tpv#add-an-allowed-payer-account) and the [Delete an Allowed Payer](/api/payments/smart-collect-tpv#delete-an-allowed-payer-account) APIs.
