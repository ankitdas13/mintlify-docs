...
description: "The customer can make a transaction to the bank account via a simple NEFT, RTGS or IMPS transaction from their preferred internet banking portal."
---
...
### 3. How do I know the allowed payers list?
       ...
       You can fetch the `allowed_payers` list using [Webhook](/webhooks/smart-collect) or API response.
...
### 4. With which banks can I open a Current/Escrow Account?
         ...
         You can open a [Current account](/x/account-types/current-account)/[Escrow account](/x/account-types/escrow) with Yes Bank or Axis Bank. 
        

    
### 5. How do I make payouts from the Escrow/Current account?

         You can make payouts using the [Payouts APIs](/x/payouts/api).
...
### 8. How is the fee for Smart Collect 2.0 collected?
         ...
         We collect the Smart Collect 2.0 usage fee from your [Fee Credits](/payments/dashboard/account-settings/credits#fee-credits). If your Fee Credits are low, we charge it from your [settlement](/payments/settlements#settlement-cycle).
