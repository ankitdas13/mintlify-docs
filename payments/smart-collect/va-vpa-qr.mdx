...
description: "For years, businesses have been collecting large payments from customers in the form of bank transfers. Collecting is easy, but reconciling payments is a tough task as it is prone to human errors and increases administrative costs."
---
...
Payments made using Smart Collect largely follow existing Razorpay [payment flow](/payments/payment-gateway/how-it-works). If you are new to Razorpay, it is recommended to understand this flow before you proceed to read the document.
...
<Step title="Step 2">
You create virtual bank accounts, virtual UPI IDs or QR code tagged to the customer.
</Step>
<Step title="Step 3">
Share account details (such as account number, IFSC and Name), UPI ID or QR code with the customer.
</Step>
<Step title="Step 4">
Customer:
- Adds the bank account as a beneficiary on their preferred netbanking portal and transfers the money using NEFT, RTGS or IMPS
...
<Step title="Step 5">
Payment deposited in these Virtual Accountss, UPI ID or QR code is settled into your bank account linked with Razorpay.
</Step>
</Steps>
...
You can consider a payment to be successful only when you receive the notification from Razorpay. You can check the [payment status on the Dashboard](/payments/smart-collect/va-vpa-qr/dashboard/create#view-payments). Also, you can choose to configure [webhooks](/payments/smart-collect/va-vpa-qr/notification) and subscribe to the `virtual_account.credited` event to receive notifications when customers make payments.
...
When first created via [Dashboard](/payments/smart-collect/va-vpa-qr/dashboard/create) or [API](/payments/smart-collect/va-vpa-qr/api/create), a Virtual Accounts is said to be in the `active` status. That is, it is ready to accept payments.

## Closed

A Virtual Accounts can be closed in two ways:
- Automatically, by using the `close_by` option at the time of Virtual Accounts creation, via [Dashboard](/payments/smart-collect/va-vpa-qr/dashboard/create) or [API](/payments/smart-collect/va-vpa-qr/api/create).
...
Once the account is in `closed` state, customers cannot make payments to that account.
 ...
 See [API Endpoints](/payments/smart-collect/va-vpa-qr/api) for more details.
