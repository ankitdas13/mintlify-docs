...
The request format for Customer Identifier creation recently underwent a change, and the updated format can be found [here](/api/payments/smart-collect). The request format given below will eventually be deprecated.
...
With the exception of the `Create` API, the request format for all other API endpoints remains the same, and can be checked [here](/api/payments/smart-collect).
