...
description: "Late authorised payments are rare situations, as only less than 0.5% of the total payments get late authorised. The payments that are **late authorised** and not captured are auto-refunded in 5 days."
---
...
Auto refunds are issued within 5 days. However, if you want to issue a refund instantly, you can capture the payment and issue a refund as per your requirements. Know more about [Refunds](/payments/refunds).
...
This is the easiest way to handle late authorisation. This feature gives you control over which payments should be auto-captured and which payments should be auto-refunded by allowing you to:
  ...
  - Setup custom timeouts for automatic and [manual capture](/payments/payments/capture-settings/api).
  - Manually capture all payments.

Know more about [Payment Capture settings](/payments/payments/capture-settings).
...
- Allows you to auto-capture payments based on the parameters passed in the [Orders API](/payments/payments/capture-settings/api). Capture values passed in the Orders API take precedence over the Payment Capture settings configured on the Dashboard.
...
You can send email notifications to your customers about the payment status, asking them to take further action to deliver the service. You can set up webhooks from the Dashboard. Know more about [webhooks](/webhooks).
...
### 1. Is Late Authorisation of payment specific to Razorpay?
     ...
     - [Webhooks](/webhooks)
     - [Payment capture settings on the Dashboard](/payments/payments/capture-settings).
...
- [Late Authorisations](/payments/payments/late-authorisation)
