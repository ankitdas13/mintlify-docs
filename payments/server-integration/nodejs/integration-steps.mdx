...
description: "Start accepting domestic and international payments from customers on your website using the Razorpay Payment Gateway. Razorpay has developed the Standard Checkout method and manages it. You can configure **payment metho"
---
...
- Know about the [Payment Flow](/payments/payment-gateway/how-it-works) and follow these integration steps:
...
### 1.2 Create an Order in Server
     ...
     - You can create an order using the [Orders API](#api-sample-code) in the **app.js** file. It is a server-side API call.  Know how to [authenticate](/api/authentication#generate-api-keys) Orders API.  
...
You can capture payments automatically with one-time [Payment Capture setting configuration](/payments/server-integration/nodejs/integration-steps#3-go-live-checklist) on the Dashboard.
...
### 1.2.3 Response Parameters
            ...
            Descriptions for the response parameters are present in the [Orders Entity](/api/orders/entity) table.
          

       
### 1.2.4 Error Response Parameters

            The error response parameters are available in the [API Reference Guide](/api/orders#error-response-parameters).
...
You can also integrate the Razorpay Checkout with [React.js](/payments/server-integration/nodejs/troubleshooting-faqs#3-can-i-integrate-razorpay-checkout-with-reactjs) using the sample code.
...
Test the integration using these [test cards](/payments/server-integration/nodejs/integration-steps#2-test-integration).
...
: `string` If you are accepting recurring payments using Razorpay Checkout, you should pass the relevant `subscription_id` to the Checkout. Know more about [Subscriptions on Checkout](/api/payments/subscriptions#checkout-integration).

`subscription_card_change` _optional_
: `boolean` Permit or restrict customer from changing the card linked to the subscription. You can also do this from the [hosted page](/payments/subscriptions/payment-retries#update-the-payment-method-via-our-hosted-page). Possible values:
...
: `boolean` Determines if you are accepting [recurring (charge-at-will) payments on Checkout](/api/payments/recurring-payments) via instruments such as emandate, paper NACH and so on. Possible values:
...
`config` _optional_
: `object` Parameters that enable checkout configuration. Know more about how to [configure payment methods on Razorpay standard checkout](/payments/payment-gateway/web-integration/standard/configure-payment-methods).
...
### 1.3.5 Configure Payment Methods *(Optional)*
          ...
          - You can configure the order or make certain payment methods prominent. Know more about configuring payment methods. Know more about [configuring payment methods](/payments/payment-gateway/web-integration/standard/configure-payment-methods).
...
### 1.5 Verify Payment Signature
     ...
     1. Create a signature in your server using the following attributes:
        ...
        ---
        `key_secret` | Available in your server. The `key_secret` that was generated from the  [Dashboard](/api/authentication#generate-api-keys).  
...
On the Dashboard, ensure that the payment status is `captured`. Refer to the payment capture settings page to know how to [capture payments automatically](/payments/payments/capture-settings).
...
Razorpay offers a range of [payment products](/payments) to meet your business requirements. Visit our  [GitHub repository](https://github.com/razorpay/razorpay-node) for the sample codes.
...
[Troubleshooting and FAQs](/payments/server-integration/nodejs/troubleshooting-faqs)
