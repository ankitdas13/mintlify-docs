...
You can capture payments automatically with one-time [Payment Capture setting configuration](/payments/server-integration/php/integration-steps#3-go-live-checklist) on the Dashboard.
...
1. Edit the `[KEY_ID]` inside the html file. Use the test keys while testing your application.  Know how to [Generate API keys](/api/authentication#generate-api-keys).  
2. Edit the `[KEY_ID]` and `[KEY_SECRET]` in **config.php**. 

- Test your integration using these [test cards](/payments/server-integration/php/integration-steps#2-test-integration).
...
Razorpay offers a range of [payment products](/payments) to meet your business requirements. Visit our [GitHub repository](https://github.com/razorpay/razorpay-php) for sample codes.
...
[Troubleshooting and FAQs](/payments/server-integration/php/troubleshooting-faqs)
