...
description: "Yes, you can. You do not need a website or mobile app to use Payment Links. You can create and send Payment Links in 3 easy steps:"
---
...
### 2. Can I create a Payment Link using which customers can pay an amount of their choice?
     ...
     No. You can create Payment Links with fixed amounts only. Customers cannot enter an amount of their choice. Use [Payment Pages](/payments/payment-pages) if you want to allow customers to enter an amount of their choice.
...
### 8. Can I set a minimum upfront payment amount when I enable Partial Payments for a Payment Link on the Dashboard?
     ...
     Currently, you can set a minimum upfront amount only through APIs. Refer to this [API endpoint](/api/payments/payment-links#request-parameters) for more information. To enabled this feature for your account, raise a ticket to the support team.
...
### 10. Can I automatically send an invoice to customers after they complete payment via a Payment Link using Razorpay Invoices?
     ...
     No, you cannot send an invoice automatically. [Razorpay's Invoices](/payments/invoices) is designed to initiate payments, not for post-payment invoicing. You need to handle invoice generation on your end after receiving payment confirmation.
    

  
### 11. Can I accept international payments using Payment Links?

     Yes, you can accept international payments using Payment Links. Know more about [international payments](/payments/international-payments).
    

  
### 12. Can I use a Payment Link to accept payments from multiple customers?

     No, you can only accept payments from a single customer using a Payment Link. Use [Payment Pages](/payments/payment-pages) if you want to use one link to accept payments from multiple customers.
...
### 16. Can I automate Payment Link generation from Salesforce and send them via WhatsApp?
     ...
     Yes, you can automate Payment Link generation using the [Payment Links API](/api/payments/payment-links/create-standard). Integrate the API into your Salesforce workflow to programmatically create links based on your business logic.
...
### 1. I have received a Payment Link that looks suspicious and fraudulent. How can I report it?
     ...
     **Additional Steps** 
     ...
     - File a dispute against the business. Know more about [Disputes](/payments/disputes).
