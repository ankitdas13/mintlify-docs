...
description: "In the Checkout form designed to meet your business needs and branding, you can display Low Cost EMI Offers to attract a broader customer base by reducing the upfront cost barrier, leading to increased sales and higher average order values."
---

You can decide the cost to subvent for each EMI tenure while [creating an offer](/payments/international-payments/accept-international-payments-from-indian-customers/standard-integration/offers/low-cost-emi/create) and the customer bears the remaining cost. This approach ensures that customers enjoy the benefits of EMI at an affordable cost, helping you minimise the overall cost.

<Info>
**New to Custom Checkout Integration?**
</Info>

If yes, know more about the [custom integration flow](/payments/payment-gateway/web-integration/custom).

## Prerequisites

Before integrating Low Cost EMI offers for your custom Checkout, run through this checklist:

1. Understand the [payment flow](/payments/payments#payment-life-cycle).
2. Generate the API keys from the [Dashboard](/payments/dashboard/account-settings#api-keys).
...
The procedure for integrating Custom Checkout on your website is explained in the [Custom Integration document](/payments/payment-gateway/web-integration/custom). However, the procedure varies while passing the offers in the payment details.

<Steps>
<Step title="[Create a Low Cost EMI Offer](#step-1-create-a-low-cost-emi-offer)">
[Create a Low Cost EMI Offer](#step-1-create-a-low-cost-emi-offer)
</Step>
<Step title="[Create an Order and Pass Offer_id](#step-2-create-an-order-and-pas...">
[Create an Order and Pass Offer_id](#step-2-create-an-order-and-pass-offer-id)
</Step>
<Step title="[Show the Offer on Checkout](#step-3-show-the-offer-on-checkout)">
[Show the Offer on Checkout](#step-3-show-the-offer-on-checkout)
</Step>
<Step title="[Submit Payment Details](#step-4-submit-payment-details)">
[Submit Payment Details](#step-4-submit-payment-details)
</Step>
</Steps>

## Step 1: Create a Low Cost EMI Offer

You can create a Low Cost EMI offer from the [Dashboard](/payments/international-payments/accept-international-payments-from-indian-customers/standard-integration/offers/create).
...
This is mandatory only in cases where you want to associate an offer or offers with the Order or you had not selected the [Show Offer on Checkout](/payments/international-payments/accept-international-payments-from-indian-customers/standard-integration/offers/create#offer-validity) while creating the offer from the Dashboard.
...
For example, you can view the image below to see how Low Cost EMI offers appear on [Razorpay Standard Checkout](/payments/payment-methods/emi/low-cost-emi).
...
Know more about [Custom Checkout](/payments/payment-gateway/web-integration/custom).
...
- From the Dashboard.
- By [configuring webhooks](/webhooks).
- By polling our [payment APIs](/api#fetch-payments-based-on-orders).
...
- [Create Low Cost EMI Offers](/payments/international-payments/accept-international-payments-from-indian-customers/standard-integration/offers/low-cost-emi/create)
