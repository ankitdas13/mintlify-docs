...
- [Load a Wallet](/payments/wallet/wallet-operations#load-a-wallet): Customer wants to add ₹ 500 to the wallet.
- [Accept Payments from a Wallet](/payments/wallet/wallet-operations#accept-payments-from-a-wallet): Customer wants to pay ₹ 500 from the wallet.
- [Refund to a Wallet](/payments/wallet/wallet-operations#refund-to-a-wallet): Merchant wants to refund ₹ 500 to a wallet.
- [Send Cashback to a Wallet](/payments/wallet/wallet-operations#send-cashback-to-a-wallet): Merchant wants to send ₹ 500 as a cashback to a wallet.
- [Make a Payout](/payments/wallet/wallet-operations#make-a-payout)

## Prerequisites

- [Set up your Razorpay account](/payments/set-up).
- [Generate API Keys](/api/authentication#generate-api-keys) from Razorpay Dashoboard.
...
<Steps>
<Step title="Step 1">
Customer proceeds to [Checkout](/payments/payment-gateway/web-integration/standard#checkout-form) from the normal payment flow for an amount, say ₹ 500, using the selected payment mode (**Net Banking**/**Debit card**/**Credit card**/**UPI**).
</Step>
<Step title="Step 2">
You authenticate the customer in your backend and make a [capture payment](/api/payments#capture-a-payment) request for the payment made and the amount gets credited to your account.
</Step>
<Step title="Step 3">
You make a [transfer payment](/payments/wallet/api-reference#transfer-a-payment) request of ₹ 500 to the customer’s wallet.
</Step>
<Step title="Step 4">
...
Refer to the [API Reference](/payments/wallet/api-reference#transfer-a-payment) to see the APIs with the examples that are used in this operation.
...
<Step title="Step 2">
You authenticate the customer in your backend and make a [create payment](/payments/wallet/api-reference#create-a-payment-to-a-customer-s-wallet) request with the `customer_id`.
</Step>
<Step title="Step 3">
You make a [capture payment](/api/payments#capture-a-payment) request for the payment of ₹ 500.
</Step>
<Step title="Step 4">
...
Refer to the [API reference](/payments/wallet/api-reference#create-a-payment-to-a-customer-s-wallet) to see the APIs with the examples that are used in this operation.

## Refund to a Wallet

Follow the below steps to refund to a wallet when payment is made from a wallet:

1. You make a [Refund to wallet](/payments/wallet/api-reference#refund-to-a-wallet) request for a payment of ₹ 500 with the `payment_id`.
...
Refer to the [API Reference](/payments/wallet/api-reference#refund-to-a-wallet) to see the APIs with the examples that are used in this operation.

## Send Cashback to a Wallet

Follow the below steps to send cashback to a wallet:

1. You make a [direct transfer](/payments/wallet/api-reference#direct-transfer) request of ₹ 500 with the `customer_id`.
...
Refer to the [API Reference](/payments/wallet/api-reference#direct-transfer) to see the APIs with the examples that are used in this operation.
...
Refer to the [API Reference](/payments/wallet/api-reference#payout-from-customer-s-wallet) to make a payout.
