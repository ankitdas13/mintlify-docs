...
Refer to the [API Reference](/api) guide to understand the basic concepts of API.

<Info>

**Handy Tips**

Before creating a wallet, you must create a [customer](/api/customers#create-a-customer).
...
If the customer's wallet has an insufficient balance for the requested payment, the API returns an error. The customer must [load sufficient amount in the wallet](/payments/wallet/wallet-operations#load-a-wallet) to complete the transaction.
...
`utr`
: `string` A unique transaction reference (UTR) number generated for all transactions. You can obtain UTR from the [`payout.updated` webhook payload](/webhooks).
