...
### Prerequisites
         ...
         1. Set up your [Razorpay account](/payments/set-up), if you have not done already.
         2. Contact your dedicated support POC to enable the TPV feature for your account.
         3. [Generate API keys](/api/authentication#generate-api-keys) required to authenticate API requests sent to Razorpay servers.
         4. Check the [best practices](/payments/third-party-validation/best-practices).
...
-  On payment failure, the customer is requested to retry payment at Checkout.
         ...
         Know more about the[Checkout Form Fields](/payments/payment-gateway/web-integration/standard/integration-steps#checkout-options).
...
### 1.4 Handle Payment Success and Failure
         ...
         If you used **Sample Code with Handler Function**:
                 ...
                 Know more about[the error parameters.](/errors#response-parameters)
...
Authorised payments can be automatically captured. You can auto-capture all payments [using global settings](/payments/payments/capture-settings#auto-capture-all-payments) on the Dashboard.
...
Payment capture settings work only if you have integrated with Orders API on your server side. Know more about the [Orders API](/api/orders#create-an-order).

         - **Manually capture payments**
         
Each authorised payment can also be captured individually. You can manually capture payments using:
             - [Payment Capture API](/api/payments#capture-a-payment)
             - [Dashboard](/payments/payments/dashboard#manually-capture-payments)

         Know more about [Capture Settings for payments](/payments/payments/capture-settings).
