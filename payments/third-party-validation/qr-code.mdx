...
description: "Generate TPV QR codes for your securities business using UPI intent deep links. Since TPV-enabled QR codes are not available as a direct product feature, create a deep link via S2S APIs and convert it to a QR code using third-party libraries."
---
...
- Ensure the [Third-Party Validation](/payments/third-party-validation) feature is activated on your account.

## Sample Code

After creating an order using the [Orders API](/api/orders/create), use the following API call to generate a UPI intent deep link and convert the returned link into a QR code:
...
: `string` Order ID generated via [Orders API](/api/orders/create).
...
<Step title="Step 3">
Use any QR code generation library to [convert this link into a QR code](#convert-upi-intent-deep-link-to-qr-code).
</Step>
<Step title="Step 4">
Display the QR code to your customers.
</Step>
<Step title="Step 5">
[Poll Payment APIs](/api/payments/fetch-with-id) to check the payment status.
</Step>
</Steps>
