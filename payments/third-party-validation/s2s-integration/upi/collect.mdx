...
description: "With UPI payments, your customers can make payments using a Virtual Payment Address (VPA) without the need to enter the bank account details. In the UPI Collect flow, customers enter their registered VPA at checkout, open the UPI PSP app and complete the payment."
---
...
<Steps>
<Step title="You should validate the VPA before initiating the payment request....">
You should validate the VPA before initiating the payment request. Know more about[VPA Validation](/payments/payment-gateway/web-integration/custom/features/validate-vpa).
</Step>
<Step title="You should add a custom UPI Collect expiry based on the business re...">
...
<Step title="You should use the [Saved VPA](/payments/payment-methods/upi/saved-...">
You should use the [Saved VPA](/payments/payment-methods/upi/saved-vpa) feature provided by Razorpay to provide a better customer experience and have a better conversion.
</Step>
</Steps>
...
- [Generate API Keys from the Dashboard](/api/authentication#generate-api-keys) if you have not done already.
- Configure the [payment capture settings](/payments/payments/capture-settings) on the Dashboard.
...
<Steps>
<Step title="[Create a customer or fetch the customer for whom VPA should be sav...">
[Create a customer or fetch the customer for whom VPA should be saved.](#step-11-create-a-customer)
</Step>
<Step title="[Create an order.](#step-12-create-an-order)">
[Create an order.](#step-12-create-an-order)
</Step>
<Step title="[Validate the VPA entered by the customer.](#step-13-validate-the-vpa)">
[Validate the VPA entered by the customer.](#step-13-validate-the-vpa)
</Step>
<Step title="[Initiate a payment.](#step-14-initiate-a-payment)">
...
`notes` _optional_
: `json object` Key-value pairs that can hold additional information about the payment. Refer to the [Notes](/api/understand#notes) section of the API Reference Guide.
...
<Steps>
<Step title="[Create an Order.](#step-21-create-an-order)">
[Create an Order.](#step-21-create-an-order)
</Step>
<Step title="[Fetch all tokens of a customer.](#step-22-fetch-vpa-tokens-of-a-cu...">
[Fetch all tokens of a customer.](#step-22-fetch-vpa-tokens-of-a-customer)
</Step>
<Step title="[Initiate a payment with the token selected by the customer.](#step...">
[Initiate a payment with the token selected by the customer.](#step-23-initiate-a-payment-using-vpa-token)
</Step>
<Step title="[Verify Payment Signature](#step-24-verify-the-payment-signature)">
