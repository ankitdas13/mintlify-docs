...
description: "Given below are FAQs on the Tokenisation guidelines issued by RBI that businesses must adopt by Sep 2022:"
---
...
<Steps>
<Step title="Step 1">
[Fetch All Saved Cards for A Customer](/payments/payment-gateway/web-integration/custom/features/saved-cards/scenario-1#33-fetch-all-tokens-of-customer).
</Step>
<Step title="Step 2">
[Fetch A Specific Saved Card for a Customer](/payments/payment-gateway/web-integration/custom/features/saved-cards/scenario-1#34-fetch-card-properties-of-an-existing-token).
</Step>
<Step title="Step 3">
[Fetch Card Properties of Individual Token](/payments/payment-gateway/web-integration/custom/features/saved-cards/scenario-1#34-fetch-card-properties-of-an-existing-token).
</Step>
<Step title="Step 4">
[Fetch all tokens of a customer](/payments/payment-gateway/web-integration/custom/features/saved-cards/scenario-1#33-fetch-all-tokens-of-customer).
</Step>
<Step title="Step 5">
[Individual Token API](/payments/payment-gateway/web-integration/custom/features/saved-cards/scenario-1#34-fetch-card-properties-of-an-existing-token).
</Step>
<Step title="Step 6">
[Fetch expanded card details of a payment](/payments/payment-gateway/web-integration/custom/features/saved-cards/scenario-1#34-fetch-card-properties-of-an-existing-token).
</Step>
<Step title="Step 7">
[Fetch card details of a payment](/api/payments#fetch-card-details-of-a-payment).
</Step>
</Steps>
...
- Instant  Refunds  for  payments  with  tokenized  cards  will  be possible  for  VISA  Debit  Cards  only. Refunds  for  all  other  card payments will be made via Razorpay’s [normal refunds](/payments/refunds/normal).
...
You can still provide refunds to your customers using [normal refunds.](/payments/refunds/normal)

#### 21. What changes must I expect in my reports?
For all the [standard reports](/payments/dashboard/reports) downloaded from the Dashboard, and the custom reports configured by your support POC, the changes will be as follows. These will be applicable for both older reports as well as newer reports getting generated.
