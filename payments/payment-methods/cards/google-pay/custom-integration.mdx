...
description: "Using this feature, you can allow your customers to make payments on your Android app using the cards they have saved on Google Pay."
---

To support Google Pay Cards on your custom checkout implementation:

1. [Show Google Pay as a separate option when Google Pay is set-up on your customer’s device](/payments/payment-methods/cards/google-pay#onboarding-and-integration).
...
## Prerequisites
- You should have already integrated [Razorpay Custom Checkout](/payments/payment-gateway/android-integration/custom) on your Android app.
- Complete the onboarding process mentioned [here](/payments/payment-methods/cards/google-pay).

## Integration Steps

<Steps>
<Step title="Step 1">
[Add or update the Google SDKs](#step-1-add-or-update-the-google-sdks)
</Step>
<Step title="Step 2">
[Update the Razorpay Custom Integration SDK](#step-2-update-razorpay-custom-integration-sdk)
</Step>
<Step title="Step 3">
[Add dependencies in the Build Gradle file](#step-3-add-dependencies-in-build-gradle-file)
</Step>
<Step title="Step 4">
[Add metadata in the Android Manifest file](#step-4-add-metadata-in-android-manifest-file).
</Step>
<Step title="Step 5">
[Instantiate and initialize the Razorpay Android Custom SDK](#step-5-instantiate-and-initialize-razorpay-android-custom)
</Step>
<Step title="Step 6">
[Implement Orders API in your server-side](#step-6-implement-orders-api-in-your-server-side).
</Step>
<Step title="Step 7">
[Fetch payment methods](#step-7-fetch-payment-methods).
</Step>
<Step title="Step 8">
[Check if Google Pay Cards is setup on the customer's device](#step-8-check-if-google-pay-cards-is).
</Step>
<Step title="Step 9">
[Set up the WebView](#step-9-set-up-the-webview).
</Step>
<Step title="Step 10">
[Submit payment data and handle success and error events](#step-10-submit-payment-data-and-handle-success).
</Step>
<Step title="Step 11">
[Store fields in the server](#step-11-store-the-fields-in-the-server).
</Step>
<Step title="Step 12">
[Verify payment signature](#step-12-verify-payment-signature).
</Step>
</Steps>
...
$order  = $client->order->create([
  ...
  'currency'        => 'INR'// [See the list of supported currencies](/payments/international-payments#supported-currencies).)
]);
```csharp: .NET
...
Know [how to integrate the different payment methods](/payments/payment-gateway/android-integration/custom/payment-methods) using Razorpay Android Custom SDK in detail.
