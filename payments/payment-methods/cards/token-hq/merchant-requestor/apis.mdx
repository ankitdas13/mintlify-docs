...
[Initiate Payment on Razorpay with token created on another PA/PG](#3-initiate-payment-on-razorpay-with-token-created).
</Step>
<Step title="[Save card to vault token while making a payment on Razorpay](#4-sa...">
[Save card to vault token while making a payment on Razorpay](#4-save-card-to-vault-token-while-making).
</Step>
</Steps>
...
`customer_id`
: `string` This is the Razorpay customer id. You can create token for a specific customer using their customer id. Use the [Customers API](/payments/customers) to create customer id. This is an optional parameter.
...
- The token is suspended for at least one of the token service providers.
  - `deactivated`- The token attains this state if the token is not active/suspended for any one of the token service providers and is deactivated for at least one token service provider. Know about the complete list of [token states](/payments/payment-methods/cards/token-hq/merchant-requestor/token-lifecycle).
...
: `string` The unique identifier of the customer created using [Customers API](/payments/customers).
