...
description: "Use this API to directly process payments with tokens saved on another PA/PG."
---
...
The payment processing flow is the same as mentioned [here](/payments/payment-gateway/s2s-integration/json/v2). However, you need to make the following changes:
...
The payment processing flow is the same as mentioned [here](/payments/payment-gateway/s2s-integration/payment-methods#emi). However, you need to make the following changes:
- Pass the token number and token expiry values instead of the card number and card expiry values.
- Pass the cryptogram (TAVV) in the `cryptogram_value` field.
