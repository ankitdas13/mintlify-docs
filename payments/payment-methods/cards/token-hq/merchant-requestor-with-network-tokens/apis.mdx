...
description: "According to recent Payment Acquirer (PA)/ Payment Gateway (PG) guidelines from RBI, businesses cannot save their customers' card numbers and other card data on their servers. Razorpay TokenHQ is a RBI-compliant solution that allows you to save customer credentials with card networks and card-issuing banks. You can use Razorpay Optimizer to route payments through the PA/PG of your choice."
---
...
<Steps>
<Step title="[Tokenise cards](#1-tokenise-cards).">
[Tokenise cards](#1-tokenise-cards).
...
<Step title="[Initiate payment using token saved with Razorpay](#2-initiate-paym...">
[Initiate payment using token saved with Razorpay](#2-initiate-payment-using-token-saved-with-razorpay).
</Step>
<Step title="[Process a payment on another PA/PG with token created on Razorpay]...">
[Process a payment on another PA/PG with token created on Razorpay](#3-process-a-payment-on-another-pa-pg).
</Step>
<Step title="[Initiate Payment on Razorpay with token created on another PA/PG](...">
[Initiate Payment on Razorpay with token created on another PA/PG](#4-initiate-payment-on-razorpay-with-token-created).
</Step>
<Step title="[Save card to vault token while making a payment on Razorpay](#5-sa...">
[Save card to vault token while making a payment on Razorpay](#5-save-card-to-vault-token-while-making).
</Step>
</Steps>
...
### Entity Parameters
      ...
      `customer_id`
      : `string` This is the Razorpay customer id. You can create token for a specific customer using their customer id. Use the [Customers API](/api/customers) to create customer id. This is an optional parameter.
...
**Handy Tips**
 ...
 Know about the complete list of [token states](/payments/payment-methods/cards/token-hq/merchant-requestor-with-network-tokens/token-lifecycle).
...
### Request Parameters
      ...
      : `string` The unique identifier of the customer created using [Customers API](/api/customers).
...
### Request Parameters
      ...
      : `string` The unique identifier of the order created for this payment. Create an order using the [Orders API](/api/orders).
...
Know more about the [S2S Integration payment flow](/payments/payment-gateway/s2s-integration/json/v2).
