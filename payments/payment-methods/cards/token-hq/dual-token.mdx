...
description: "TokenHQ, a complete RBI-compliant solution from Razorpay, enables you to save customer credentials as tokens with card networks and card-issuing banks."
---
...
`customer_id`
: `string` This is the Razorpay customer id. You can create a token for a specific customer using their customer id. Use the [Customers API](/payments/customers) to create a customer id. This is an optional parameter.
...
Descriptions for the response parameters are present in the [Token Entity parameters table](/payments/payment-methods/cards/token-hq/dual-token#token-entity).
