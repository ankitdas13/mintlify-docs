...
<Step title="[Save card request along with payment](#2-save-card-request-along-w...">
[Save card request along with payment](#2-save-card-request-along-with-payment)
</Step>
<Step title="[Initiate a payment using a previously created token](#3-initiate-p...">
[Initiate a payment using a previously created token](#3-initiate-payment-using-saved-token)
</Step>
<Step title="[Process a Payment on another PA/PG with Token Created on Razorpay]...">
[Process a Payment on another PA/PG with Token Created on Razorpay](#4-process-a-payment-on-another-pa-pg)
</Step>
</Steps>
...
: `string` The overall status for the token. Possible values:
 ...
 Know about the complete list of [token states](/payments/payment-methods/cards/token-hq/merchant-requestor-with-network-tokens/token-lifecycle#overall-token-states).
