---
title: "Bank Transfer"
description: "`amount` _mandatory_"
---
...
All bank transfer payments are auto-captured. [Manual capture](/payments/payments/capture-settings#manually-capture-payments) of payments is not supported.
...
The bank transfer payment method will appear for the [Payment Gateway](/payments/payment-gateway/web-integration/standard) and products such as Payment Links and Payment Pages.
...
### Step 1: Track Checkout Modal Using `ondismiss` Function
     ...
     If you have integrated with [Razorpay Standard Checkout](/payments/payment-gateway/web-integration/standard), you must implement the `ondismiss` function to track the lifecycle of the Checkout modal. This displays the `close` icon, which the customer can use to exit the Checkout.
...
If you are using [Android SDK](/payments/payment-gateway/android-integration/standard), you can rely on the "payment cancelled by user" error response to track the lifecycle of the Checkout modal.
...
### Step 3: Subscribe to Webhook Event
   ...
   You must subscribe to the `virtual_account.credited` webhook event on the Dashboard to receive notifications whenever customers make payments using bank transfers. Know how to [set up webhooks](/webhooks/setup-edit-payments).
...
### Step 4: Track Checkout Modal Using `ondismiss` Function
   ...
   (Only if you are using [Standard Checkout](/payments/payment-gateway/web-integration/standard#manual))
...
### Step 6: Subscribe to Webhook Event
    ...
    You must subscribe to the `virtual_account.credited` webhook event on the Dashboard to receive notifications whenever customers make payments using bank transfers. Know how to [setup webhooks](/webhooks#set-up-webhooks).
...
- [Hosted Integration for Bank Transfer](/payments/payment-methods/bank-transfer/hosted-integration)
- [Custom Integration for Bank Transfer](/payments/payment-methods/bank-transfer/custom-integration)
