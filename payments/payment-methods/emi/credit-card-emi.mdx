...
description: "Razorpay Checkout supports EMIs on credit cards issued by major banks. EMI is available by default on Razorpay Standard Checkout. No additional integration is needed."
---
...
Following is the list of supported Payment Partners for Credit Card EMI and the minimum order amount stipulated by them:
   ...
   Check the standard [credit card interest rates charged by major banks](/payments/payment-methods/emi/faqs#1-what-are-the-standard-credit-card-interest).
...
To fire this API, you must provide your [KEY_ID] for authorization. Your [KEY_SECRET] is not required and should not be passed. Know how to generate the [API Keys](/api/authentication#generate-api-keys).
...
<Step title="Step 5">
Enter the relevant details. The eligibility depends on the customer's card's BIN (first 6 digits). If the card is not eligible, the customers can pay the full amount.
</Step>
<Step title="Step 6">
...
[Credit Card EMI FAQs](/payments/payment-methods/emi/faqs#credit-card-emi).
