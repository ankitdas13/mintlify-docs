...
description: "You can display Bajaj Finserv No Cost EMI offers to your customers by integrating with Razorpay custom checkout."
---
...
- Refer to our [web custom integration document](/payments/payment-gateway/web-integration/custom).
...
<Steps>
<Step title="[Create No Cost EMI offers](#step-1-create-no-cost-emi-offers).">
...
<Step title="[Fetch payment methods](#step-3-fetch-payment-methods).">
[Fetch payment methods](#step-3-fetch-payment-methods).
</Step>
<Step title="[Invoke checkout and pass order_id and other options](#step-4-invok...">
[Invoke checkout and pass order_id and other options](#step-4-invoke-checkout-and-pass-order-id).
</Step>
<Step title="[Store fields in your server](#step-5-store-fields-in-your-server).">
[Store fields in your server](#step-5-store-fields-in-your-server).
</Step>
<Step title="[Verify payment signature](#step-6-verify-payment-signature).">
...
The list of checkout parameters is available [here](/payments/payment-gateway/web-integration/custom/build-integration).
...
You can subscribe to a Webhook event that is generated when a certain event happens in our server. When one of those events is triggered, Razorpay sends the Webhook payload to the configured URL. [Know how to set up Webhooks.](/webhooks)

#### Poll APIs

You can retrieve the status of the payments by polling our [Payment APIs](/api/payments#fetch-multiple-payments).
