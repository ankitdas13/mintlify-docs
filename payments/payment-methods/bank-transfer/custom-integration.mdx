...
description: "You can now accept payments from customers in the form of online bank transfers using the Razorpay Custom Checkout."
---
...
- [Generate the API keys](/api/authentication#generate-api-keys).
- Integrate with [Razorpay Custom Checkout](/payments/payment-gateway/web-integration/custom).
...
<Step title="Step 2">
[Add the `fetchVirtualAccount` method to the custom checkout integration](#step-2-add-fetchvirtualaccount-method-to-custom-checkout).
</Step>
<Step title="Step 3">
[Subscribe to the virtual account credited webhook event](#step-3-subscribe-to-webhook-event).
</Step>
</Steps>
...
: `string` The unique identifier of the customer. Learn how to create a customer using the [Customers API](/api/customers). This parameter is mandatory if you want to associate the virtual account with a specific customer.
...
`notes`
: `json object` Any custom notes you might want to add to the virtual account can be entered here. Refer [Notes section of the API Reference Guide](/api/understand#notes) to learn more.

`customer_id`
: `string` Unique identifier of the customer to whom the virtual account is linked. Refer the [Customer API](/api/customers) section to learn more.
...
: `json object` Configuration of desired receivers for the virtual account.
    ...
    `notes`
    : `json object` Any custom notes you might want to add to the virtual bank account or virtual UPI ID can be entered here. Refer [Notes section of the API Reference Guide](/api/understand#notes) to learn more. This parameter appears in the response only when `bank_account` is passed as the receiver `type`.
...
You must subscribe to the `virtual_account.credited` webhook event on the Dashboard to receive notifications whenever customers make payments using bank transfers. Learn how to [setup webhooks](/webhooks#set-up-webhooks).
