...
description: "Secure Server-to-Server (S2S) integration that enables seamless and reliable processing and a smooth payment experience for your customers."
---
...
<Steps>
<Step title="[Sign up](https://dashboard.razorpay.com/#/access/signup) for a Raz...">
...
<Step title="Follow the [Razorpay S2S Integration documentation](/payments/payme...">
Follow the [Razorpay S2S Integration documentation](/payments/payment-gateway/s2s-integration).
</Step>
</Steps>
     ...
     : `array` Payment capture settings for the payment. The options sent here override the [account level auto-capture settings](/payments/payments/capture-settings) configured using the Dashboard.
      ...
      : `array` Use this array to determine the expiry period for automatic and [manual capture](/payments/payments/capture-settings/api) of payments and the refund speed in the case of non-capture.
          ...
          : `string` Refund speed for payments that were not captured (automatically or manually). Possible values:
            - `optimum`: We try to process the refund instantly. We charge a small fee for this. If it is not possible to process an instant refund, we will process a normal refund in 5-7 working days. [Know more about instant refunds](/payments/refunds#how-instant-refunds-work).
