...
description: "Razorpay Checkout Blocks is a client-side JavaScript SDK that enables businesses to offer express checkout experiences with digital wallets such as Apple Pay, Google Pay and more, directly on their own checkout page. The SDK abstracts the complexities of integrating individual wallet providers, allowing you to go live with single-click payments. Know more about [Razorpay Payments](https://razorpay.com/docs/payments)."
---
...
- **Order Creation via Backend**: Your server must be able to create orders using the [Orders API](/api/orders/create) and pass the `order_id` to the frontend.
...
Before initiating a payment, create an order on your backend using the [Razorpay Orders API](/api/orders/create). Pass the resulting `order_id` to your frontend to initialise the SDK.
...
Refer to the [Orders API documentation](/api/orders/create) for detailed request and response parameters.
