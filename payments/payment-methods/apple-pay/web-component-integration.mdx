...
- Ensure you have your API `Key id` and `Key Secret` readily available. Know how to generate [API Keys from the Dashboard](/payments/dashboard/account-settings/api-keys#generate-api-keys).
...
### Step 3: Create an Order on Your Server
       ...
       The parameter descriptions and errors are present in the [Create an Order API](/api/orders/create).
...
### Step 5: Verify the Payment Signature on Your Server
       ...
       Verify them using the standard [Razorpay signature verification process](/payments/server-integration/python/integration-steps#14-verify-payment-signature).
...
[Apple Pay WKWebView Integration](/payments/payment-methods/apple-pay/wkwebview-integration)
