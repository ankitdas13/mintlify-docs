...
description: "The first transaction needs to go through the 2FA process. Further charges can be made automatically, without 2FA."
---
...
### 5. The order status remains "Pending Payment", even after a successful Razorpay payment. Why is WooCommerce not updating my subscription payments?
     ...
     - [Subscriptions plugin for WooCommerce](/payments/subscriptions/plugins/woocommerce)
     - [Subscriptions plugin for WooCommerce Build Integration](/payments/subscriptions/plugins/woocommerce/build-integration)
...
### 6. Is WooCommerce Subscriptions plugin dependent on the Razorpay WooCommerce plugin? What are the compatible versions?
     ...
     Yes, the WooCommerce Subscriptions plugin depends on the Razorpay WooCommerce plugin. The WooCommerce Subscriptions plugin is fully compatible with Razorpay WooCommerce plugin equal to or greater than v2.4, including the latest v3.0 release. Know more about [plugin dependencies](/payments/subscriptions/plugins/woocommerce#plugin-dependencies).
...
### 1. Can I update the plan amount or duration of Subscription when it is live?
     ...
     Yes. You can update details such as the plan amount, duration and quantity of a Subscription, even if it is live. Refer to the [Update Subscription section](/payments/subscriptions/update) for more details.
...
### 2. When can I update a Subscription?
     ...
     You can only update a Subscription when it is in `authenticated` and `active` states. There is no state change when you update a Subscription. Know about [Updating a Subscription](/payments/subscriptions/update).
...
### 2. Can all Subscriptions be paused?
     ...
     Subscriptions in the `active` state can be paused. You cannot pause a Subscription in any other state. Refer to the [life cycle](/payments/subscriptions/states) section for more details.
...
### 3. How can I pause a Subscription?
     ...
     - [From the Dashboard](/payments/subscriptions/pause-resume-cancel#pause-subscription-via-the-dashboard).
     - [Using APIs](/api/payments/subscriptions#pause-a-subscription).
...
### 2. How can I resume a Subscription?
     ...
     - [From the Dashboard](/payments/subscriptions/pause-resume-cancel#resume-subscription-via-the-dashboard).
     - [Using APIs](/api/payments/subscriptions#resume-a-subscription).
...
### 1. Can I cancel a Subscription?
     ...
     - [From the Dashboard](/payments/subscriptions/pause-resume-cancel).
     - [Using APIs](/api/payments/subscriptions#cancel-a-subscription).
...
### 3. How can I know when my customer cancels a Subscription?
     ...
     You can set up the [subscription.cancelled](/webhooks/subscriptions#subscription-cancelled) webhook event to get a notification when your customer cancels their Subscription. Know more about [subscribing to a webhook event](/payments/subscriptions/subscribe-to-webhooks).
...
### 3. Which banks and apps support UPI Subscriptions?
     ...
     **Supported Banks** 
     ...
     Refer to the [list of banks that support UPI Autopay](/payments/recurring-payments/upi/supported-banks#supported-banks).
...
### 1. Which banks are live on the cards, and what does this mean for businesses?
     ...
     Refer to the [list of banks that support cards](/payments/subscriptions/supported-banks-apps#cards).
...
### 4. What happens to debits of Subscriptions plans of banks that are not live by September 30, 2021?
     ...
     **Alternate Approach** - Register for a new Subscription
     ...
     Refer to the [Subscriptions document](/payments/subscriptions) for more information.
...
All the steps mentioned below will be taken care of by Razorpay and banks. No additional effort is required from businesses and customers.
     ...
     To process debits of amounts above ₹15,000 (upto ₹1,00,000 for [certain merchant categories](/payments/subscriptions/supported-banks-apps#supported-mccs)):
     1. Razorpay will initiate the debit 36 hours before the actual debit date.
...
### 23. What banks support recurring payments via RuPay cards?
     ...
     RuPay SI requires payment processors/aggregators and banks to work with NPCI to enable the support. NPCI is continuously working with more banks to support recurring payments on RuPay cards. Refer to the [Supported Banks and Apps](/payments/subscriptions/supported-banks-apps#cards) document for the list of supported banks.
...
### 6. How long does it take to process UPI payments?
     ...
     ***** Due to [payment retries](/payments/subscriptions/payment-retries#retry-model-for-upi), this may extend to T+1.
...
For certain MCCs, the maximum amount limit for one transaction is ₹1,00,000. Refer to the [the list of supported MCCs](/payments/subscriptions/supported-banks-apps#supported-mccs), issuing banks and PSP applications.
...
### 20. Is QR Code supported for UPI Autopay?
     ...
     Yes, [QR Code](/payments/subscriptions/qr-codes) is supported on Standard Checkout to collect Subscription payments. The customers can scan the QR using any PSP app and make the payment.
...
### 4. Which banks have enabled Subscription through Emandate?
     ...
     Currently, Subscription through Emandate is available for a few banks. Refer to the [Supported Banks and Apps](/payments/subscriptions/supported-banks-apps) page for more information.
...
### 1. When can I link an offer to a Subscription?
     ...
     know more about [Subscription Offers](/payments/subscriptions/offers).
...
### 6. Can I link an offer to a Subscription after it has gone live?
     ...
     Yes. You can [link an offer to a Subscription after it has gone live](/payments/subscriptions/offers/link#link-offer-to-an-existing-subscription).
...
### 1. Can I accept Subscription payments in currencies other than INR?
     ...
     Yes. You can accept Subscription payments in any of the [supported currencies](/payments/international-payments#supported-currencies).
...
### 2. In what currency are international payments settled?
     ...
     Refer to our [International Payments page](/payments/international-payments) for more details.
