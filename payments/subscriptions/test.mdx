...
description: "Check the Prerequisites and steps to test the Subscriptions and the various flows."
---

## Prerequisites

- Use your test [KEY_ID] and [KEY_SECRET] while testing. This can be generated from the Dashboard by [generating API keys](/payments/dashboard/account-settings/api-keys) in the test mode.
...
### 1. Configure a Webhook
     ...
     Before you create Subscriptions, [configure a test webhook](/webhooks/setup-edit-payments) to receive notifications regarding Subscriptions.
    

  
### 2. Create a Plan

     Create a Plan from your [Dashboard](/payments/subscriptions/create-plans#create-a-plan) and use them in your code while creating Subscriptions. You can also create Plans using [Razorpay Subscriptions APIs](/api/payments/subscriptions/create-plan).
...
### 3. Create a Subscription
     ...
     **Example**
     ...
     If you create a Subscription using the Dashboard, it can only be created as a [Subscription Link](/payments/subscriptions/create-subscription-links).

     

     

     You can also create Subscriptions using [APIs](/api/payments/subscriptions/create-subscription). A sample request and response for the creation of **Subscription A** and **Subscription B** is given below.
...
### 4. Make Authorisation Payment
      ...
      The above code only contains the minimum required options that need to be sent to initiate a payment, that is, only `key` and `subscription_id` are mandatory. Check the [checkout fields](/payments/payment-gateway/web-integration/standard/integration-steps#123-checkout-options) for a complete list of options that can be passed.
...
### Payment Amount
          ...
          To make a successful payment, the payment response contains a few fields carrying information about the Subscription that has been authenticated. Check the [payment verification section](/payments/subscriptions/integration-guide#payment-verification) to know more about how this payment response can be authenticated.
...
### Customer changes card on payment failure
      ...
      If there are any incomplete charges, you have to manually attempt to charge the card again from the [Dashboard](/payments/subscriptions/manually-charge-card).
...
### Complete and Cancelled Subscriptions
     ...
     - Cancelling a Subscription using [Dashboard](/payments/subscriptions/pause-resume-cancel#cancel-a-subscription-via-the-dashboard) or [API](/api/payments/subscriptions#cancel-a-subscription).
