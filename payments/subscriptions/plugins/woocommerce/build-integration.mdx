...
description: "Follow the steps given below to integrate Razorpay Payment Gateway to your WooCommerce-enabled WordPress site using the Razorpay Subscription for WooCommerce plugin."
---
...
1. [Install the Razorpay Subscriptions for WooCommerce Plugin](/payments/subscriptions/plugins/woocommerce/build-integration#step-1-install-razorpay-subscriptions-for-woocommerce-plugin).
1. [Configure WooCommerce Settings](/payments/subscriptions/plugins/woocommerce/build-integration#step-2-configure-woocommerce-settings).
1. [Create a Subscriptions product using WooCommerce](/payments/subscriptions/plugins/woocommerce/build-integration#step-3-create-a-subscriptions-products).
1. [Test integration](/payments/subscriptions/plugins/woocommerce/test-integration).

### On Razorpay Dashboard

1. [Enable Webhooks](/payments/subscriptions/plugins/woocommerce/build-integration#you-can-track-the-payment-status-in-three). 
...
<Steps>
<Step title="In the **WordPress Admin Dashboard**, navigate to **Plugins** → **A...">
In the **WordPress Admin Dashboard**, navigate to **Plugins** → **Add New**.
</Step>
<Step title="Search for **Razorpay Subscriptions for WooCommerce** and click **I...">
Search for **Razorpay Subscriptions for WooCommerce** and click **Install Now**.
...
#### Manual Installation
<Steps>
<Step title="[Download a ZIP file](https://github.com/razorpay/razorpay-woocomme...">
[Download a ZIP file](https://github.com/razorpay/razorpay-woocommerce-subscriptions/releases) of the Razorpay WooCommerce Subscriptions Plugin from the repository on Github.
</Step>
<Step title="Unzip this file and upload the contents in `wp-content` → Plugins.">
Unzip this file and upload the contents in `wp-content` → Plugins.

</Step>
<Step title="The plugin now appears in your **WordPress Dashboard** → **Plugins*...">
The plugin now appears in your **WordPress Dashboard** → **Plugins** folder.
</Step>
<Step title="Click **Activate**.">
Click **Activate**.
</Step>
...
2. On the **Razorpay Payment Gateway** page, fill in the following field details and click **Save**:
    ...
    ---
    Payment Action | To automatically capture successful payments, select `Authorize and Capture` option in the drop-down. Select `Authorize` if you want to capture payments manually from the [Razorpay Dashboard](/payments/payments/dashboard#manual-capture-of-payments) or using [API](/api/payments/capture).
    ---
    Order Completion Message | Enter the message that must be displayed after an order is successfully placed.
    ---
    Enable Webhook | Ensure that **Enable Razorpay Webhook** option is selected. [Set up webhooks](/webhooks/setup-edit-payments) from the Dashboard.
    ---
...
<Steps>
<Step title="In the WordPress Dashboard, navigate to **Products** → **Add New**.">
In the WordPress Dashboard, navigate to **Products** → **Add New**.
</Step>
<Step title="Enter the product name and add a brief description.">
Enter the product name and add a brief description.
</Step>
<Step title="In the Product data section, enter the following details and click...">
In the Product data section, enter the following details and click **Publish**:
...
- [Make a test transaction](/payments/subscriptions/plugins/woocommerce/test-integration) to ensure that the integration is working properly.
...
[Step 2: Test Integration](/payments/subscriptions/plugins/woocommerce/test-integration)
