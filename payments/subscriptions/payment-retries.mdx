...
description: "Recurring payments for a Subscription are auto-debited based on the scheduled day that you defined. However, these payments could fail."
---
...
### Update Payment Method on Our Hosted Page
     ...
     You can use our ready-made hosted page solution to handle payment failures when you attempt an auto-charge. Here is how the hosted page handles payment failure:
      ...
      - After the customer clicks the **Update Payment Method** button, the checkout page appears as shown. The customer can choose a card (of [supported banks](/payments/subscriptions/supported-banks-apps)), UPI or Emandate (of [supported banks](/payments/subscriptions/supported-banks-apps#emandate)) to make the payment. If the payment is successful, the Subscription moves back to the `actived` state.
