...
description: "Optimizer is compatible with all types of Razorpay Checkout, including [Standard](/payments/payment-gateway/web-integration/standard), [Custom](/payments/payment-gateway/web-integration/custom), and [Server-to-Server (S2"
---
...
### 1. Which types of Razorpay Checkout does Optimizer support?
         ...
         Optimizer is compatible with all types of Razorpay Checkout, including [Standard](/payments/payment-gateway/web-integration/standard), [Custom](/payments/payment-gateway/web-integration/custom), and [Server-to-Server (S2S)](/payments/payment-gateway/s2s-integration) integrations.
...
### 3. Which payment gateways are supported by Optimizer?
         ...
         Optimizer supports a wide range of payment gateways and aggregators, including PayU, Pinelabs, Billdesk, Cashfree, Paytm, and many others. Find the complete [list of supported gateways](/payments/optimizer/add-payment-providers#supported-payment-providers).
        

    
### 4. What payment modes are supported by Optimizer for different payment gateways?

         Optimizer supports a range of payment modes, including cards, UPI, Netbanking, wallets, and EMI, providing a comprehensive payment solution. Optimizer also facilitates seamless refund and settlement processes, enhancing the overall payment experience. Know more about [supported gateways and methods](/payments/optimizer/supported-gateways-aggregators).
...
### 5. What is the process to onboard other gateways on Optimizer?
         ...
         2. Follow the [steps to add a payment gateway](/payments/optimizer/add-payment-providers).
...
- Please note that the prerequisites may vary for each gateway. Know more about [prerequisites for various gateways](/payments/optimizer/add-payment-providers#supported-payment-providers).
...
### 8. How can I identify the payment gateway used for a payment in the payment fetch API or webhook events?
         ...
         When Optimizer is enabled for your account, an additional parameter `gateway_provider` is included in the payment fetch response and webhook events. This parameter specifies the payment gateway through which the payment was made. Know more about [Payment API and Webhook events](/payments/optimizer/get-started#payments-api).
...
### 1. Where can I pass the custom identifier in the order? 
         ...
         The custom identifier needs to be passed in the Payments or Orders API. Know more about [custom identifiers](/payments/optimizer/create-custom-rule#custom-identifiers).
...
### 1. How does Optimizer support refunds?
         ...
         - [Normal Refunds](/payments/refunds/normal): For normal refunds, the refund is initiated with the respective payment gateway and depends on the availability of funds in the gateway's balance.
         - [Instant Refunds](/payments/refunds/instant): For instant refunds, If Razorpay processes the payment, we initiate an instant refund through Razorpay. If another payment gateway processes the payment, we initiate the refund through that specific gateway.
