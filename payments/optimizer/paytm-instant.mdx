...
description: "Go live with your Paytm Payment Gateway account instantly using the **Instant** integration mode. This is a beta release and supports limited [payment methods](/payments/optimizer/paytm-instant#supported-payment-methods). Follow the steps below to onboard Paytm Instant (beta) as a payment provider."
---
...
If you want to offer all payment methods and support refunds from the Dashboard, please use the [S2S integration mode](/payments/optimizer/paytm-s2s).
...
You might be generating a unique Order id or Receipt for every order which can be passed to Paytm via [Orders API](/api/orders#orders-api). 
...
<Step title="Step 2">
Razorpay passes this value to Paytm in the `extendInfo.udf1` parameter.
</Step>
<Step title="Step 3">
Write to your Paytm Relationship Manager to show the parameter `extendInfo.udf1` in the Dashboard and reports as per your use case.
</Step>
</Steps>
