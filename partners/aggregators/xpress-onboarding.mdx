...
description: "Xpress onboarding allows partners to quickly onboard sub-merchants to accept payments on Razorpay with minimal KYC."
---
...
- Razorpay verifies the KYC details and partially activates the sub-merchant account. The account moves to `activated_kyc_pending` state. The sub-merchant can start accepting payments up to GMV (Gross Merchandise Value) limit of â‚¹50,000 (or â‚¹5,00,000 if GST verified) using Payment Links and QR Codes.
...
- [Update Product Configuration API](/api/partners/product-configuration-xpress-onboarding#update-a-product-configuration)
...
Create an account for the sub-merchant using the [Create an Account API](/api/partners/account-onboarding/create) by passing the `no_doc_onboarding=true` parameter. This triggers the Xpress Onboarding flow.
...
API | Description
---
[Create an account](/api/partners/account-onboarding/create) | API to create a sub-merchant account.
---
[Fetch an account](/api/partners/account-onboarding/fetch) | API to view sub-merchant account details.
---
[Update an account](/api/partners/account-onboarding/update) | API to update the sub-merchant account details.
---
[Delete an account](/api/partners/account-onboarding/delete) | API to delete a sub-merchant account.
...
API | Description
---
[Create a stakeholder](/api/partners/stakeholder/create) | API to create a stakeholder.
---
[Fetch a stakeholder](/api/partners/stakeholder/fetch-with-id) | API to retrieve and view the stakeholder account details.
---
[Fetch all stakeholders](/api/partners/stakeholder/fetch-all) | API to retrieve all stakeholders for a given account.
---
[Update a Stakeholder](/api/partners/stakeholder/update) | API to update the stakeholder account details.
 
    

### 3. View and Accept Terms and Conditions

Before activating the sub-merchant and requesting for relevant product configurations, the sub-merchant needs to view and accept the Terms and Conditions. You can fetch these Terms and Conditions links using the [Fetch Terms and Conditions API](/api/partners/terms-conditions/fetch) to share them with the sub-merchant.

You can also accept Terms and Conditions for the sub-merchant using the [Request a Product Configuration API](/api/partners/product-configuration-xpress-onboarding#request-a-product-configuration). 
...
API | Description
---
[Accept Terms and Conditions](/api/partners/product-configuration-xpress-onboarding#request-a-product-configuration) | API to accept terms and conditions.
---
[Fetch Terms and Conditions](/api/partners/terms-conditions/fetch) | API to retrieve terms and conditions.

    

### 4. Configure Products to Accept Payments

You can use the [Product Configuration APIs](/api/partners/product-configuration-xpress-onboarding) to configure and activate Razorpay products for a sub-merchant account according to their requirements.
...
API | Description
---
[Request a Product Configuration](/api/partners/product-configuration-xpress-onboarding#request-a-product-configuration) | API to request a product configuration.   
---
[Update a product configuration](/api/partners/product-configuration-xpress-onboarding#update-a-product-configuration) | API to update a configured product.
---
[Fetch a Product](/api/partners/product-configuration-xpress-onboarding#fetch-a-product) | API to fetch a configured product.

    

### 5. Upload Supporting Documents

You can submit the required documents for sub-merchant onboarding using the [Documents APIs](/api/partners/upload-document) listed in the table below.
...
API | Description
---
[Upload account documents](/api/partners/upload-document/upload-account-documents) | API to upload sub-merchant's business documents.
---
[Upload stakeholder documents](/api/partners/upload-document/upload-stakeholder-documents) | API to upload stakeholder's signatory documents.
---
[Fetch account documents](/api/partners/upload-document/fetch-account-documents) | API to fetch account documents.
---
[Fetch stakeholder documents](/api/partners/upload-document/fetch-stakeholder-documents) | API to fetch stakeholder documents.
...
- **Transaction Events**: These are events related to payment transactions performed by the sub-merchants. You can configure these events at a sub-merchant level.  View [webhook events](/partners/aggregators/onboarding-api/webhooks#transaction-events) and [payload details](/webhooks).
...
### Available Webhook Events and Descriptions
  ...
  View [payload details](/webhooks/partners#onboarded-using-apis).
...
### To subscribe to webhook events:
        ...
        2. Navigate to **Partner's Dashboard** â†’ **Settings** â†’ **Webhooks** to subscribe to any of the events mentioned in the following section.
        3. Click **Manage**.
...
You can use the [Webhooks APIs](/api/partners/webhooks) to receive event notifications or subscribe to events happening in a sub-merchant's account, such as payments, orders, invoices and so on.
...
API | Description
---
[Create a Webhook](/api/partners/webhooks/create) | API to create a webhook.
---
[Fetch a Webhook](/api/partners/webhooks/fetch-with-id) | API to retrieve and view the webhook details.
---
[Fetch all Webhooks](/api/partners/webhooks/fetch-all) | API to retrieve all webhooks.
---
[Update a Webhook](/api/partners/webhooks/update) | API to update the webhook details.
---
[Delete a Webhook](/api/partners/webhooks/delete) | API to delete a webhook.
