---
title: "Smart Collect"
description: "Razorpay Smart Collect enables you to create virtual accounts to accept large payments from your customers in the form of bank transfers via NEFT, RTGS and IMPS."
---
...
Consider these [prerequisites](/payments/smart-collect#prerequisites) before getting started with Smart Collect.

## Create a Virtual Account API

Given below is the sample code for the Create a Virtual Account API. Pass the `account_id` of the sub-merchant using `X-Razorpay-Account` in the header. Refer to the [Smart Collect API documentation](/api/payments/smart-collect).
...
: `json object` Configuration of desired receivers for the virtual account.
    ...
    `qr_code` _optional_
    : `json object` Descriptor details for the QR code. This is to be passed only when `qr_code` is passed as the receiver `types`. [Know about the request parameters to be passed to create a QR code](/api/qr-codes).

`description` _optional_
: `string` A brief description of the virtual account.

`customer_id` _optional_
: `string` Unique identifier of the customer to whom the virtual account must be tagged. Refer to the [Customer API](/api/customers) documentation to learn how to create a customer.

`notes` _optional_
: `json object` Any custom notes you might want to add to the virtual account can be entered here. Refer to the [Notes section of the API Reference Guide](/api/understand#notes) to know more.
...
- [Smart Collect API documentation](/api/payments/smart-collect).
