---
title: "Payment Gateway (Import Flow - Liberalised Remittance Scheme)"
...
You can try out our APIs on the Razorpay Postman Public Workspace. Fork the workspace and test the APIs with your [Test API Keys](/api/authentication#test-mode-api-keys).
...
- Expired API key.
 | The API keys must be active and entered correctly with no whitespace before or after the keys. Know how to [Generate API keys](/api/authentication#generate-api-keys).
       ---
...
After the order is created, an `order_id` is generated, for example, `order_NGrgEcmYJsfUyl`. You must pass this `order_id` in the checkout code to associate this order with the payment. Learn more about [Order and Payment states](/payments/orders#order-states).
...
Test your integration using these [test cards](/payments/international-payments/accept-international-payments-from-indian-customers/standard-integration/test-integration#Cards).
...
- UPI
- Cards
- [Recurring](/payments/international-payments/accept-international-payments-from-indian-customers/standard-integration/recurring-payments) 
...
: `string` Unique identifier of customer. Used for:
    - [Local saved cards feature](/payments/payment-methods/cards/features/saved-cards#save-card-details).
...
- [Invoice Collection](/payments/international-payments/outward-remittances/standard-integration/invoice-collection)
