...
description: "According to new [Card-on-File tokenisation guidelines from RBI](https://razorpay.com/blog/card-on-file-tokenisation-all-you-need-to-know/), businesses cannot save their customer's card credentials (card number and other card data) on their own servers."
---
...
: `string` Order ID generated via [Razorpay Orders API](/api/orders).
...
: `string` Name of the payment method. Possible values are:
    ...
    - `emi` (default)
    - `upi` (default)
    - `bank_transfer` (requires [approval](https://razorpay.com/support/#request) and [integration](/payments/payment-methods/bank-transfer))
    - `cardless_emi` (requires [approval](https://razorpay.com/support/#request) and [integration](/payments/payment-methods/emi/cardless-emi))
    - `paylater` (requires [approval](https://razorpay.com/support/#request) and [integration](/payments/payment-methods/pay-later))
...
: `string` Bank code. List of available banks enabled for your account can be fetched via [**methods**](/payments/payment-gateway/web-integration/custom#step-3-fetch-payment-methods).
...
: `string` Wallet code for the wallet used for the payment. Possible values:
    ...
    - `amazonpay` (requires [approval](https://razorpay.com/support/#request) and [integration](/payments/payment-methods/wallets/amazon-pay))
    - `paypal` (requires [approval](https://razorpay.com/support/#request). [Learn more](/payments/payment-methods/wallets/paypal))
    - `phonepeswitch` (requires [approval](https://razorpay.com/support/#request) and [integration](/payments/payment-gateway/web-integration/custom/phonepe-switch))
...
### Other Token APIs
- [Token IIN API](/payments/payment-methods/cards/token-hq/merchant-requestor/iin-validation) 
- [PAR API](/api/payments/cards/fingerprints) 
- [Fetch Card IIN Information API](/api/payments/cards/iin-api)
