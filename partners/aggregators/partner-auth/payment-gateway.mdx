...
description: "Follow these steps to integrate with the Razorpay Payment Gateway:"
---

<Steps>
<Step title="[Create an order using Orders API](#step-1-create-an-order-using-or...">
[Create an order using Orders API](#step-1-create-an-order-using-orders-api).
</Step>
<Step title="[Add the Razorpay Checkout code to your website](#step-2-add-razorp...">
[Add the Razorpay Checkout code to your website](#step-2-add-razorpay-checkout-code-to-website).
</Step>
<Step title="[Store fields in your server](#step-3-store-fields-in-your-server).">
[Store fields in your server](#step-3-store-fields-in-your-server).
</Step>
<Step title="[Verify payment signature](#step-4-verify-payment-signature).">
[Verify payment signature](#step-4-verify-payment-signature).
</Step>
<Step title="[Verify payment status](#step-5-verify-payment-status).">
...
Integrate with [Orders API](/api/orders) to create an order. Given below is the sample code for Create an Order API. Pass the `account_id` of the sub-merchant using `X-Razorpay-Account` in the header.
...
Check [our SDKs](/payments/server-integration) for the supported platforms.
...
You can view the payments received from customers on the sub-merchant's  Dashboard. Also, you can retrieve the status of the payments by polling our [APIs](/api).
...
- [Razorpay Payment Gateway: Web Standard Integration Guide](/payments/payment-gateway/web-integration/standard).
