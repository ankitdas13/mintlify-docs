...
description: "Information security is critical for businesses handling financial transactions online. Being a technology-first online finance company, we ensure that every transaction made using Razorpay products is secure."
---
...
- Implement [Two-Factor Authentication (2FA)](/payments/dashboard/account-settings/manage-team#2-step-verification-team-level) across your team for additional security.
...
- The [Razorpay API secret](/api/authentication#generate-api-keys) is not included in the final Android or iOS build.
...
While integrating with our [Payments API](/api/payments), ensure that you:
- Use the [Capture a Payment API](/api/payments#capture-a-payment) to assert the payment status.
- Fetch the amounts of captured payments only from the backend or a trusted source.

### Orders API

While integrating with our [Orders API](/api/orders), ensure that:

- [Payments are auto-captured](/payments/payments/capture-settings#auto-capture-all-payments) from the Dashboard.
...
- [Razorpay IPs](/security/whitelists#webhook-ips) are added to all the whitelisted webhook requests.
...
- Define [user roles](/payments/dashboard/account-settings/manage-team#standard-user-roles) for team members based on their usage of the Dashboard.
- Implement [Two-Factor Authentication (2FA)](/payments/dashboard/account-settings/manage-team#2-step-verification-team-level) on all your Razorpay accounts.
- Never share Razorpay Two-Factor Authentication OTPs among employees.

## Plugin Integration

Implement the below best practices while integrating with Razorpay using [third-party plugins](/payments/payment-gateway/ecommerce-plugins).
