...
description: "**POST** `/v1/fund_accounts`"
---
...
* solution: Enter the correct Contact ID. Check if this contact already exists in your RazorpayX account using [Fetch Contact id API](/api/x/contacts/fetch-with-id) or use the [search option](/x/dashboard/global-search) on the Dashboard. Or find the Contact ID: - In the response body of [create a Contact](/api/x/contacts/create) API.
- On the [Contacts](/x/contacts) Dashboard.
