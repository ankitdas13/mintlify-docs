...
description: "**GET** `/v1/fund_accounts`"
---
...
: `string` The fund account type to be retrieved. Possible values:
    - `bank_account`
    - `vpa`
    - `card` (if [payouts to cards](/x/payouts/cards) is enabled on your account)
...
* solution: Re-check or find the Fund Account ID: - In the response body of [create a Fund Account](/api/x/fund-accounts) API.
