...
Ensure you [allowlist IPs](/x/dashboard/allowlist-ip) and pass the [idempotency key](/api/x/payout-idempotency/make-request) to make a successful payout.

 Payout to Cards is only available for PCI DSS compliant merchants. To enable the feature, [raise a request](/x/support) on the RazorpayX Dashboard. Refer to the [payouts to cards](/api/x/payouts-cards) documentation. 
...
- Contact
  ...
  - Use the [Update Contact API](/api/x/contacts/update) if you want to make changes to an existing contact.
...
To understand the status of the payouts, refer to [Payout Status Details](/errors/x/payout-status-details).
...
: `object` Contact and fund account details to which the payout was made.
  ...
  : `string` The type of fund account being created. It can be a `bank_account`, `vpa`, `card`.
    ...
    : `object` Details of the credit card that is being used to create the fund account.
        ...
        `issuer`
        : `string` The name of bank that issued the card. For example, `HDFC`. Refer to the [Supported Banks and Payout Modes section](/api/x/payouts-cards#supported-banks-and-payout-modes) section for more details.
...
`mode`
: `string` The mode used to make the payout. Refer to the [Payouts section](/x/payouts#payout-modes) for more details. Available modes:
    - `NEFT`
    - `RTGS`
    - `IMPS`
    - `UPI`
