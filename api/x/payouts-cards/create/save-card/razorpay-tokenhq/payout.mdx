...
Use this endpoint to create a payout to fund account type `card` by saving the card with [Razorpay TokenHQ](/payments/payment-methods/cards/token-hq/razorpay-requestor-with-network-tokens).
