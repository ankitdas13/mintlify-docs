...
Use this endpoint to create a fund account type `card` by saving the card with [Razorpay TokenHQ](/payments/payment-methods/cards/token-hq/razorpay-requestor-with-network-tokens).
