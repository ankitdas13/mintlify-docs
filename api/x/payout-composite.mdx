...
description: "Composite API allows you to make three requests in a single API call. No OTP authorization is required when creating a payout using APIs. Ensure you [allowlist IPs](/x/dashboard..."
---

Composite API allows you to make three requests in a single API call. No OTP authorization is required when creating a payout using APIs. Ensure you [allowlist IPs](/x/dashboard/allowlist-ip) and pass the [idempotency key](/api/x/payout-idempotency/make-request) to make a successful payout. 
...
Fork the Razorpay Postman Public Workspace and try the Payout Composite APIs using your [Test API Keys](/x/dashboard/api-keys).
...
[About Payouts API](/api/x/payouts)
