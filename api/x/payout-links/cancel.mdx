...
description: "**POST** `/v1/payout-links/:id/cancel`"
---

Use this endpoint to cancel a Payout Link. You can only cancel Payout Links in the `issued` state. Know more about the [Payout Link Life Cycle section](/x/payout-links/life-cycle).
...
: `string` The unique identifier of the contact's fund account to which the payout will be made. For example, `fa_00000000000001`. 
 ...
 Fund Account id is returned only when the Payout Link moves to the [`processing` state](/x/payout-links/life-cycle) after the contact provides their account details.

`payout_id`
: `string` The unique identifier for the payout made to the contact. For example, `pout_00000000000001`. 
 
 This value is returned only when the Payout Link moves to the [`processed` state](/x/payout-links/life-cycle) after the contact would receive the amount in their account.
