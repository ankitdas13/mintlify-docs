...
: `integer` Possible values:
  - `1` : Retrieves Orders for which payments have been authorized. Payment and order states differ. Know more about [payment states](/payments/payments#payment-life-cycle).
...
For more information about creating transfers using orders, refer to the [Route](/api/payments/route) section of the [Route API](/webhooks/route) documentation.
- `virtual_account`: Returns the virtual account details created for each order. 
For more information about creating Virtual Accounts, refer to the [Smart Collect API](/api/payments/smart-collect)
