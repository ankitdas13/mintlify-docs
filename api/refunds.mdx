---
title: "Refunds"
description: "Make full or partial refunds to customers. You can initiate refunds only on those payments that are in `captured` state. A payment in `authorized` state is auto-refunded if not `captured` within 3 days of creation. You can create and manage refunds using APIs or from the [Dashboard](/payments/refunds#dashboard-actions)."
---

Fork the Razorpay Postman Public Workspace and try the Refunds APIs using your [Test API Keys](/api/authentication#generate-api-keys).
...
[Sample Payloads](/webhooks/refunds)
