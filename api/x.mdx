---
title: "Get Started"
description: "Post sign up, account activation and KYC verification you can start using APIs to make payouts. To make a payout, you must:"
---
...
It is assumed that you have already added funds to your business account. This action **cannot** be performed via APIs and has to be done before you can make a payout. Refer to the business account section to know [how to add funds to your business account](/x/account-types#add-funds).
...
Know more about [API parameters](/x/apis).
...
Know more about [Test Mode](/x/dashboard/test-mode).
...
## Webhooks
[Set up Payout Webhooks](/webhooks/setup-edit-payouts)to configure and receive notifications when a [specific event occurs](/webhooks#available-events). When one of these events is triggered, we send an HTTP POST [payload](/webhooks) in JSON to the webhook's configured URL.
