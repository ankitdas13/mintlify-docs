...
description: "**POST** `/v1/payments/:id/transfers`"
---

Use this endpoint to create Transfers from captured payments. You can create and capture payments in the regular payments flow using the [Razorpay Checkout](/payments/payment-gateway/web-integration/standard) and [Payment APIs](/api/payments#capture-a-payment).
...
: `array` Details regarding the transfer.
    ...
    `on_hold`
    : `boolean` Indicates whether the account settlement for transfer is on hold. Know more about [on hold settlements](/payments/route/schedule-settlement#settlement-settings-for-linked-accounts). Possible values:
