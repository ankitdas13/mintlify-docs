...
description: "**PATCH** `/v1/payment_links/:id`"
---
...
: `boolean` Indicates whether customers can make [partial payments](/payments/payment-links/partial-payments) using the Payment Link. Partial payments are not allowed with UPI Payment Links. Possible values:
...
`notes` _optional_ 
: `object` Key-value pair that can be used to store additional information about the entity. Maximum 15 key-value pairs, 256 characters (maximum) each. For example, `"note_key": "Payment Link for Groceriesâ€`.
