...
Once you have generated an `order_id`, use it with the `token_id` to create a one-time payment. After the payment is complete (authorized or failed), the respective token will move to the `cancelled` state and you can no longer use the token. You will get the [token.cancelled](/api/payments/recurring-payments/webhooks#token-cancelled) webhook notification for the same.
