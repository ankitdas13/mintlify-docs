...
description: "The flow to complete an authorisation transaction using paper NACH is a little different from the regular recurring payment flow. The flow when using paper NACH is:"
---
...
<Step title="[Create Authorisation Payment using Razorpay APIs](#113-create-an-a...">
[Create Authorisation Payment using Razorpay APIs](#113-create-an-authorisation-payment).
</Step>
</Steps>
...
### Sample Code
      ...
      $api->customer->create(array('name' => 'Gaurav Kumar', 'email' => 'gaurav.kumar@example.com','contact'=>'+919876543210','fail_existing' => "0", 'notes'=> array('notes_key_1'=> 'Tea, Earl Grey, Hot','notes_key_2'=> 'Tea, Earl Greyâ€¦ decaf'));
      ```csharp: .NET
...
`notes` _optional_
: `object` Key-value pair that can be used to store additional information about the entity. Maximum 15 key-value pairs, 256 characters (maximum) each. For example, `"note_key": "Beam me up Scottyâ€`.
...
tokenNotes.put("notes_key_2","Tea, Earl Greyâ€¦ decaf.");
...
$api->order->create(array('amount' => 0,'currency' => 'INR','method' => 'nach','customer_id' => 'cust_1Aa00000000001','receipt' => 'Receipt No. 1',  'notes' => array('notes_key_1' => 'Beam me up Scotty','notes_key_2' => 'Engage'),'token' => array('auth_type' => 'physical','max_amount' => 10000000,'expire_at' => 2709971120,'notes' => array('notes_key_1' => 'Tea, Earl Grey, Hot','notes_key_2' => 'Tea, Earl Greyâ€¦ decaf.'),'bank_account' => array('account_number' => '11214311215411','ifsc_code' => 'HDFC0000001','beneficiary_name' => 'Gaurav Kumar','account_type' => 'savings'),'nach' => array('form_reference1' => 'Recurring Payment for Gaurav Kumar','form_reference2' => 'Method Paper NACH','description' => 'Paper NACH Gaurav Kumar'))));
...
notes.Add("notes_key_2","Tea, Earl Greyâ€¦ decaf.");
...
tokenNotes.Add("notes_key_2","Tea, Earl Greyâ€¦ decaf.");
...
```html: Custom checkout with handler functions
  ...
  const btn = document.querySelector("#btn");
          ...
          var data = {
      "amount": 100, // in currency subunits. Here 1000 = 1000 paise, which equals to â‚¹10
...
To learn about errors, refer to the FAQ [Upload the NACH File](/payments/recurring-payments/paper-nach/faqs) section.
...
`notes` _optional_
: `object` This is a key-value pair that can be used to store additional information about the entity. Maximum 15 key-value pairs, 256 characters (maximum) each. For example, `"note_key": "Beam me up Scottyâ€`.
