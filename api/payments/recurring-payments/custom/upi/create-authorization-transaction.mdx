...
The Orders API allows you to create a unique Razorpay `order_id`, for example, `order_1Aa00000000001`, that would be tied to the authorisation transaction. Refer to our detailed [Order documentation](/payments/orders) for more details.
...
### UPI Intent
     ...
     Platform | Procedure
     ---
     **mWeb** | Customers are redirected to their preferred UPI app to complete the payment. For the complete integration guide, refer to [UPI Intent on Mobile Web](/payments/payment-gateway/web-integration/custom/payment-methods/upi-intent-mweb).
     ---
     **Mobile App (WebView)** | UPI Intent requires deep link handling in your Android or iOS app to launch UPI apps from the WebView. For the complete integration guide, refer to [UPI Intent in WebView — Android](/payments/payment-gateway/web-integration/custom/features/webview/upi-intent-android) and [UPI Intent in WebView — iOS](/payments/payment-gateway/web-integration/custom/features/webview/upi-intent-ios).
     ---
...
**UPI Collect is deprecated effective 28 February 2026.** This tab is applicable only for exempted businesses. If you are not covered by the exemptions, refer to the [migration documentation](/announcements/upi-collect-migration/recurring-payments/custom-checkout) to switch to UPI Intent.
