...
description: "Once you capture a payment, Razorpay Checkout returns a `razorpay_payment_id`. You can use this id to fetch the `token_id`, which is used to create and charge subsequent payments."
---
...
Know more about [Tokens](/payments/recurring-payments/cards/integrate#fetch-card-mandate-registration-details).
...
You can also retrieve the `token_id` from the [payment.authorized webhook](/api/payments/recurring-payments/webhooks#payment-authorized).
