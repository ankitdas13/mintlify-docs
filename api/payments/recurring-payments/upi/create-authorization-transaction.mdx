...
- If you are an existing Razorpay user not covered by exemptions, you must migrate to UPI Intent or UPI QR code to continue accepting UPI Autopay registrations. For detailed migration steps, refer to the [migration documentation](/announcements/upi-collect-migration/recurring-payments/standard-checkout).
...
### UPI Intent
      ...
      Platform | Steps
      ---
      **mWeb** | Customers are redirected to their preferred UPI app to complete the payment. For the complete integration guide, refer to [UPI Intent on Mobile Web](/payments/payment-methods/upi/upi-intent#integrate-on-android-ios-and-mobile-web).
      ---
      **Mobile App (WebView)** | UPI Intent requires passing `webview_intent: true` in the checkout options and implementing deep link handling in your Android or iOS app. For the complete integration guide, refer to [UPI Intent in WebView — Android](/payments/payment-gateway/web-integration/standard/webview/upi-intent-android) and [UPI Intent in WebView — iOS](/payments/payment-gateway/web-integration/standard/webview/upi-intent-ios).
      ---
...
You can [use Webhooks to get notifications about successful payments](/api/payments/recurring-payments/webhooks#check-registration-link-status-using-webhooks) against a registration link.
