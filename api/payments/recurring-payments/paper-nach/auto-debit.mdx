...
description: "You can register a customer's mandate and charge them the first recurring payment as part of the same transaction. For this you have to pass the `first_payment_amount` parameter while creating the order."
---
...
<Step title="Create an order by passing the `customer_id` and the method as `nac...">
...
<Step title="The customer can get the form in one of the following ways to sign it:">
...
<Step title="The signed form can be uploaded in one of the following ways:">
...
<Steps>
<Step title="[Create a Customer](#111-create-a-customer).">
[Create a Customer](#111-create-a-customer).
</Step>
<Step title="[Create an Order](#112-create-an-order).">
[Create an Order](#112-create-an-order).
</Step>
<Step title="[Create Authorisation Payment using Razorpay Standard Checkout](#11...">
[Create Authorisation Payment using Razorpay Standard Checkout](#113-create-an-authorisation-payment).
</Step>
</Steps>
...
You can use the [Orders](/payments/orders) API to create a unique Razorpay `order_id` that is associated with the authorisation transaction. The following endpoint creates an order.
...
: Details related to the authorisation transaction such as max amount and bank account information. Pass a value in the `first_payment_amount` parameter if you want to auto-charge the customer the first payment immediately after authorization.
 ...
 `auth_type` _mandatory_
  ...
  : `integer` Use to set the maximum amount per debit request. Know about [maximum and default values](/payments/recurring-payments/paper-nach/faqs#3-is-there-a-limit-on-the-debit).
...
: Details related to the authorisation such as max amount and bank account information.
  ...
  `max_amount`
  : `integer` The maximum amount in paise a customer can be charged in a transaction. Know about the [maximum and default values](/payments/recurring-payments/emandate/faqs#2-what-is-the-maximum-amount-which-can).
...
You can register a customer's mandate **and** charge them the first recurring payment as part of the same transaction. Refer to the [Paper NACH section under Registration and Charge First Payment Together](/api/payments/recurring-payments/paper-nach/auto-debit#1-create-an-authorization-transaction) for more information.
...
Registration Link is an alternate way of creating an authorisation transaction. You can create a registration link using the [API](#121-create-a-registration-link) or [Dashboard](/payments/recurring-payments/create#1-create-a-registration-link).
...
- When you create a registration link, an [invoice](/payments/invoices) is automatically issued to the customer. They can use this invoice to make the authorisation payment.
...
You can [use Webhooks to get notifications about successful payments](/api/payments/recurring-payments/webhooks#check-authorization-link-status-using-webhooks) against a registration link.
...
<Steps>
<Step title="Download the pre-filled form using the Download NACH Form option on...">
Download the pre-filled form using the Download NACH Form option on the Razorpay hosted page.
</Step>
<Step title="Sign the form.">
Sign the form.
</Step>
<Step title="Upload the signed form using the Upload NACH Form option on the Raz...">
...
: Details of the authorisation payment.
  ...
  `max_amount` _optional_
  : `integer` Use to set the maximum amount, in paise, per debit request. Know about [maximum and default values](/payments/recurring-payments/paper-nach/faqs).
...
Know more about [Tokens](/payments/recurring-payments/paper-nach/integrate#fetch-nach-mandate-registration-details).
