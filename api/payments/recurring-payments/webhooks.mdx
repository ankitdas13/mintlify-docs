---
title: "Webhooks"
description: "Webhooks (Web Callback, HTTP Push API or Reverse API) automatically notify your application when specific events occur. Instead of continuously polling APIs to check for updates, webhooks push notifications directly to your server when events happen."
---
...
### Capturing Late Authorised Payments
         ...
         Sometimes, the communication between the bank and Razorpay or between you and Razorpay may not occur. This could be because of a slow network connection or your customer closing the window while processing the payment. This could lead to a payment being marked as **Failed** on the Dashboard but changed to **Authorized** at a later time. Know more about [late authorisation of payments](/payments/payments/late-authorisation).
...
- You can set up webhooks from your Dashboard and configure separate URLs for **Live** mode and **Test** mode. Know more about setting up [Payment webhooks](/webhooks/setup-edit-payments) and [Payout webhooks](/webhooks/setup-edit-payouts).
- A **Test** mode webhook receives events for your test transactions. Know more about [testing webhooks](/webhooks/validate-test).
- Webhook URLs must use ports **80** or **443** only.
- Ensure Razorpay webhook IPs are whitelisted on your server. Even if your server accepts all incoming requests, webhooks may still be blocked by cloud security groups or network configurations. Refer to [Razorpay IPs and Certificates](/security/whitelists#webhook-ips) for the complete list of webhook IP addresses.
...
<Steps>
<Step title="You can identify the duplicate webhooks using the `x-razorpay-event...">
...
<Step title="Check the value of `x-razorpay-event-id` in the webhook request hea...">
Check the value of `x-razorpay-event-id` in the webhook request header.
</Step>
<Step title="Verify if an event with the same header is processed at your end.">
...
On failure, a webhook is re-tried at progressive intervals of time, defined in the exponential back-off policy, for 24 hours. If the failures continue for 24 hours, the webhook is disabled. You need to enable the webhook from the Dashboard after fixing the errors at your end. Know more about [enabling Webhooks](/webhooks/setup-edit-payments#enabledisable-a-webhook).
...
<Steps>
<Step title="Log in to the Dashboard and navigate to **Accounts & Settings**.">
Log in to the Dashboard and navigate to **Accounts & Settings**.
</Step>
<Step title="Click **Webhooks** under **Website and app settings**.">
Click **Webhooks** under **Website and app settings**.
</Step>
<Step title="Click the **+ Add New Webhook** button.">
...
When your webhook `secret` is set, Razorpay uses it to create a hash signature with each payload. This hash signature is passed with each request under the `X-Razorpay-Signature` header that you need to validate at your end. We provide support for validating the signature in all of our [language SDKs](/payments/server-integration).
...
---
[`payment.authorized`](/api/payments/recurring-payments/webhooks#payment-authorized) | Indicates the payment has been authorized. A payment is authorized when the customer’s payment details are successfully authenticated by the bank.
---
[`payment.captured`](/api/payments/recurring-payments/webhooks#payment-captured) | Indicates when the payment has been captured.
---
[`order.paid`](/api/payments/recurring-payments/webhooks#order-paid) | Indicates the customer has successfully made the payment.
---
[`payment.failed`](/api/payments/recurring-payments/webhooks#payment-failed) | Indicates that the payment has failed. If the payment fails, you need to create an authorisation transaction again.
...
---
[`token.confirmed`](/api/payments/recurring-payments/webhooks#token-confirmed) | - Emandate
- Card
- Paper NACH
-  UPI
...
---
[`token.rejected`](/api/payments/recurring-payments/webhooks#token-rejected) | - Emandate
-  Card
- Paper NACH
-  UPI
  | Indicates that the has been rejected. If rejected, you need to create the authorisation transaction again.
---
[`token.cancelled`](/api/payments/recurring-payments/webhooks#token-cancelled) | - Emandate
- UPI
 - Card
 | Indicated the token has been cancelled.
---
[`token.paused`](/api/payments/recurring-payments/webhooks#token-paused) | UPI | Indicated the token has been paused by your customer.
...
Webhook Event | Description
---
[`invoice.paid`](/api/payments/recurring-payments/webhooks#invoice-paid) | Indicates that a registration link is successfully paid.
---
[`invoice.expired`](/api/payments/recurring-payments/webhooks#invoice-expired) | Indicates that a registration link has expired.
...
You can use these webhooks to check the status of the pre-debit notification sent to the customer when the payment method is [UPI](/api/payments/recurring-payments/upi/create-authorization-transaction) and [Cards](/api/payments/recurring-payments/cards/create-authorization-transaction). These notification webhooks are available only if you send the notification object while creating an order.
