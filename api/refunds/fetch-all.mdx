...
`currency`
: `string` The currency of a payment amount for which the refund is initiated. Check the list of [supported currencies](/payments/international-payments#supported-currencies).
...
`notes`
: `json object` Key-value store for storing your reference data. A maximum of 15 key-value pairs can be included. For example, `"note_key": "Beam me up Scottyâ€`.
