...
description: "According to NPCI guidelines, the UPI Collect flow is being deprecated from 28 February 2026."
---
...
### Desktop Web
         ...
         **Change Needed**
         ...
         1. Update your API call to use UPI Intent flow instead of UPI Collect flow. Remove `vpa` and `expiry_time` parameters from the request body. Replace `collect` with `intent` as the value of the `upi.flow` parameter. Refer to the [UPI Intent guide](/payments/payment-gateway/s2s-integration/recurring-payments/upi-intent/authorization-transaction#113-create-an-authorisation-payment) for further information.
         ...
         3. Use [webhooks to get payment status updates](/webhooks/payments).
...
- [Recurring Payments - UPI Autopay S2S Integration](/payments/payment-gateway/s2s-integration/recurring-payments/upi-intent/authorization-transaction)
