---
title: "Run Payroll"
description: "After you have finalised the salary components, you can finally execute your payroll. Ensure you refer to the [payroll checklist](/payroll/execute-payroll) before doing so."
---
...
Automated Professional Tax (PT) payments for employees in Karnataka are temporarily unavailable on Payroll. Know more about the [PT rule change](/payroll/faqs#professional-tax).
...
1. Check the following: 
    ...
    - Fulfil other components from the [execute payroll checklist](/payroll/execute-payroll).
...
You have successfully executed payroll. You cannot make [payroll modifications](/payroll/exceptional-cases#modify-salary-after-payroll-is-executed) after you execute payroll. 
...
1. Log in to the Payroll Dashboard.
<Steps>
<Step title="Navigate to **ADMIN OPTIONS** → **Pay Employees** → **Run Payroll**.">
Navigate to **ADMIN OPTIONS** → **Pay Employees** → **Run Payroll**.
</Step>
<Step title="If required, select the month you want to skip the employee's salary.">
If required, select the month you want to skip the employee's salary.
</Step>
<Step title="Search the employee's name and click on the **Edit** icon on the ri...">
Search the employee's name and click on the **Edit** icon on the right-most column.

</Step>
<Step title="Select **SKIP THIS** to skip the employee's salary for that month.">
...
1. Log in to the Payroll Dashboard.
<Steps>
<Step title="Navigate back to **Run Payroll**.">
Navigate back to **Run Payroll**.
</Step>
<Step title="Select the payroll month and the employee's profile whose salary yo...">
Select the payroll month and the employee's profile whose salary you have skipped.
</Step>
<Step title="Click **EDIT**, and click **Resume Pay**.">
Click **EDIT**, and click **Resume Pay**.
</Step>
<Step title="Finalise the payroll again.">
...
1. Log in to the Payroll Dashboard.
<Steps>
<Step title="Navigate to **Run Payroll**, and select the payroll month if required.">
Navigate to **Run Payroll**, and select the payroll month if required.
</Step>
<Step title="Search the employee for whom you want to execute payroll and click...">
Search the employee for whom you want to execute payroll and click on the **Edit** icon in the last column.
</Step>
<Step title="Click **SKIP ALL EXCEPT THIS**. This will pause all the other emplo...">
...
<Step title="Click any of the skipped employees' profiles (from step 2 to [execu...">
Click any of the skipped employees' profiles (from step 2 to [execute payroll for one employee](#execute-payroll-for-one-employee)) and then on the **Edit** icon.
</Step>
<Step title="Click **Resume all skipped**.">
...
1. On the **Revise compensation** page, modify the following as applicable: 
    1. CTC in case of default salary structure adopted by Payroll. Or you can select [custom salary structure](/payroll/salary#setup-salary-structure) as applicable to your organisation. 
...
1. Enter the revised salary details against the relevant employee's names. 
    ...
    - In the Custom Structure sheet, enter the revised CTC breakdown for each employee under [Custom Salary Structure](/payroll/salary#setup-salary-structure). 
...
1. Select the **Last working date** (LWD) from the drop-down calendar and provide a reason for dismissal. If you have enabled the [resignation module](/payroll/employees/resignation), we automatically add the LWD once the employee submits their resignation.

This initiates the full and final settlement process for the employee. Know how to [process employee's FNF](/payroll/resignation). 
...
You have three ways with which you can calculate [loss of pay](/payroll/leaves#loss-of-pay-for-unpaid-excessive-leave-work) for Unpaid/Excessive Leave Work. Know how you can make salary deductions for the same below. 
...
<Steps>
<Step title="Navigate to **Run Payroll** → **Select employee**.">
Navigate to **Run Payroll** → **Select employee**.
</Step>
<Step title="Click the **EDIT** icon under the last, right-most column.">
Click the **EDIT** icon under the last, right-most column.

</Step>
<Step title="Add the amounts under **Additions** and click **DONE**.">
Add the amounts under **Additions** and click **DONE**.
</Step>
<Step title="Click **Gross Pay** after you add a bonus to view the new payslip i...">
...
<Steps>
<Step title="Navigate back to **Run Payroll** and select the employee.">
Navigate back to **Run Payroll** and select the employee.
</Step>
<Step title="Click the **EDIT** icon under the right-most column against the emp...">
Click the **EDIT** icon under the right-most column against the employee's name.
</Step>
<Step title="Navigate to **Deductions** enter the amount for **Recovery** or the...">
Navigate to **Deductions** enter the amount for **Recovery** or the [Loss of Pay](#loss-of-pay) Days.
</Step>
</Steps>
...
If the edit icon looks uneditable and says **Final**, click **Make Changes** on the right menu to make the column editable again. Know more about [making changes to payroll](/payroll/exceptional-cases#modify-salary-after-payroll-is-executed).

### Loss of Pay
Payroll does not automatically deduct the loss of pay based on attendance. By default, attendance is considered `present` for all calendar days of the month in Payroll. Know more about [attendance](/payroll/attendance). 
...
- **Jibble Integration**: Sync the loss of pay data from the Jibble Dashboard to automatically calculate and deduct loss of pay amount. Know more about the [Jibble attendance integration](/payroll/integrations/jibble). 
...
### Custom Salary Structure
         ...
         If you use a custom salary structure, we do not prorate the employee/s LOP deduction based on the gross salary. All the salary components, except compliance payments, are prorated separately based on your organisation's [attendance setup](/payroll/faqs#xpayroll-s-loss-of-pay-calculation-is-not) to arrive at the LOP deduction amount. 
...
- [One-time Payments](/payroll/one-time-payments) 
