---
title: "Custom Payroll & Attendance Cycle"
description: "Every organisation uses an attendance cycle to record the employees' attendance data for a payroll month and pays salaries. In RazorpayX Payroll, you can set up a [payroll and a..."
---
...
You can use an attendance cycle to freeze the employees' attendance for that month and finalise the payroll before [execution](/payroll/run-payroll#execute-payroll).
...
Know more about [payroll execution](/payroll/execute-payroll). 
...
1. Select the **New cycle effective month** from the drop-down menu. You can only select an [unfinalised payroll month](/payroll/run-payroll#execute-payroll) for the updated payroll and attendance cycle to take effect.
...
1. Toggle the setting against **Payroll Autopilot**. This means that the Payroll automatically makes payroll payouts on the **Payroll Date**. 
    When disabled, you must manually [request execution](/payroll/execute-payroll).
...
- [About Account Setup](/payroll/quickstart)
- [About Execute Payroll Checklist](/payroll/execute-payroll)
