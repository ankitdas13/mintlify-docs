...
description: "You can accept customer payments using Razorpay products. By subscribing to payments webhook events you can get notified about payment state changes."
---
...
<Steps>
<Step title="We **trigger and send a `payment.failed` webhook** to your configur...">
We **trigger and send a `payment.failed` webhook** to your configured endpoint, indicating the initial payment failure.
</Step>
<Step title="If the customer retries the payment and successfully completes it (...">
...
<Step title="Subsequently, we **send a `payment.captured` webhook**, confirming...">
